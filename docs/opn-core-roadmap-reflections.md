...
## 2026-07-20 (latest) — Sprint 10 **part A** (measurement foundation: the loadgen seq-continuity verifier + the `design` scenario + the core-pinned production-shaped perf stack + the `perf/results/` discipline): built, verified live (a healthy design-shaped run, `seq_gaps=0` end to end); the profiling/fix loop + the four heavier scenarios (part B) deferred
