...
## 2026-07-20 (latest) — Sprint 11 **part B**, slice 2 (backups + the restore DRILL, roadmap item 3: a streamed `pg_dump`-to-S3 backup, a `pg_restore`-from-S3 recovery, a best-effort `mc mirror` for media, the `restore-from-backup` runbook, and — the piece the roadmap makes non-negotiable — a self-verifying end-to-end restore **drill**): built, **verified live on the dev host** (`sg docker`), the drill green — one full backup → destroy the DB volume → restore into a fresh stack → verify cycle asserting `_sqlx_migrations` (12) and the seeded message row round-trip, `opn_app` (least-priv, its real password, over TCP) reads the message back with its world set and reads **zero** with a different world (RLS intact post-restore), and **Core boots Healthy on the restored DB** (no-op migrate, serves as `opn_app`); `pg_cron` partition automation (item 2) stays HELD for Sprint 10's soak, and the operator-only pieces (Coolify's backup schedule, a real off-box S3 target, the operator's own periodic drill) are named not done

"An untested backup is a wish, not a backup" is the roadmap's own line for item 3, and the reflex of the last two slices — "ship the machinery here, the *runs* stay operator" — would have shipped four scripts and a runbook and called the drill an operator exercise. But this slice's frontier is exactly what the dev host with `sg docker` (plus the release binary and the prod image already built last slice) *can* now prove that no amount of `docker compose config` can: that a dump actually restores and the restored DB actually serves. So the drill was run, not deferred — and running it is what turned item 3 from "scripts buildable" into "backup proven recoverable." The load-bearing correctness (the dump/restore role + RLS reasoning, the failure-domain modelling, the drill's assertions) stayed on the main thread; the two genuinely-junior jobs — the read-heavy runbook and an adversarial pass over the scripts — fanned out to parallel opus agents, and the adversarial one earned its cost (the keeper below).

### What exists now

- **`opn-core/deploy/backup/lib.sh`** — shared config + three helpers (`compose()` targets the stack, `mc_run()` runs `mc` in a throwaway `minio/mc` container, `require()` fails on a missing env). Every value is env-overridable so the identical scripts run against prod or a throwaway drill stack. It also documents the load-bearing role choice: `pg_dump`/`pg_restore` run as `opn_migrate` — the compose `POSTGRES_USER`, i.e. the cluster **bootstrap superuser** (BYPASSRLS) — because only a BYPASSRLS role dumps every row and restores COPY *past* the domain tables' `FORCE ROW LEVEL SECURITY`; `opn_app` (NOBYPASSRLS) could do neither.
- **`opn-core/deploy/backup/pg-backup.sh`** — streams `pg_dump -Fc` straight into the backup S3 bucket via `mc pipe` (no temp file, no host pg/mc tools — only Docker). **Atomic publish**: streams to `<key>.partial`, then `mc mv`s to the final `.dump` only after a clean pipe, so the bucket only ever holds *complete* objects. Retention prune (default 30 d, guarded against an empty prefix and a non-numeric horizon). Meant for a Coolify Scheduled Task / cron.
- **`opn-core/deploy/backup/pg-restore.sh`** — streams a dump from S3 into `pg_restore --clean --if-exists --single-transaction`. Destructive, so a safety gate refuses a non-empty target unless `OPN_RESTORE_FORCE=1`. The single transaction means any mid-restore failure rolls the target back to its pre-restore state — never a half-dropped DB.
- **`opn-core/deploy/backup/media-mirror.sh`** — best-effort `mc mirror --overwrite` (additive, no `--remove`) of the primary media bucket to the backup target. Media loss is cosmetic (§3).
- **`opn-core/deploy/backup/restore-drill.sh`** — the proof artifact and the operator's periodic drill. Reuses `pg-backup.sh`/`pg-restore.sh` (so the real scripts are exercised) in a throwaway isolated compose project, and models the actual failure domain: only the `pgdata` volume is destroyed; the backup lives in MinIO, which survives.
- **`docs/runbooks/restore-from-backup.md`** — the sixth and final runbook (item 5 now 6/6): what/where, the full env table, scheduling, the restore procedure with its three real preconditions (`opn_app` must pre-exist for the carried GRANTs; restore-as-superuser past FORCE RLS; the force gate), and the drill as the periodic proof.
- **`opn-core/.dockerignore`** — `deploy/` added: those scripts are mounted at runtime or run on the host, never COPYed into the image, so excluding them keeps a backup-script edit from busting the cargo build cache (proved: the post-fix drill re-run recompiled **0** crates).

### Decisions closed during implementation (roadmap deviations, all ponytail)

1. **Run the drill live; do not defer it as "operator".** The last two slices' honest split was build-here / runs-there, but the *run* here is a backup+restore on Postgres+MinIO — which this dev host with `sg docker` can do in full, and which the roadmap makes mandatory ("an untested backup is a wish"). Deferring it would have shipped the one item whose whole point is being tested, untested.
2. **All pg/mc work is containerized — no host tools.** The host has no `pg_dump`/`pg_restore`/`psql`/`mc` (checked first). So pg tools run via `compose exec` into the postgres service and every `mc` call runs in a throwaway `minio/mc` container. The scripts need only Docker — which is also exactly the prod reality.
3. **Restore as `opn_migrate` (superuser), not `opn_app`; roles must pre-exist.** A single-DB `pg_dump` emits GRANTs to `opn_app` but not the role itself, and FORCE RLS blocks COPY for any non-BYPASSRLS role. So restore runs as the bootstrap superuser and leans on the `initdb` pre-seed to have created `opn_app` before the GRANTs land — the exact chain the fresh-boot slice built last session, reused here.
4. **Backup target = a different failure domain, required not defaulted-wrong.** DB loss is fatal (§3), so the DB backup MUST NOT live in the same MinIO the app writes media to. The target is env-parameterized and `require`d; the runbook states the off-box rule loudly rather than baking a same-store default that would quietly defeat the point.
5. **Atomic publish + single-transaction restore — from the adversarial review (the keeper).**
6. **`mc` creds passed by NAME only (`-e VAR`, not `-e VAR=value`) — also from the review.** Keeps `KEY:SECRET@host` out of argv / `/proc/<pid>/cmdline`.
7. **`deploy/` into `.dockerignore`.** Deploy scripts have no business in the runtime image, and excluding them stops this slice's own edits from triggering a 54 s recompile on every drill run — a real, measured cache win, not a guess.
8. **Fanned the runbook + an adversarial script review to two parallel opus juniors; kept the scripts and the live drill.** The runbook is read-heavy and welded to the finalized scripts (so it went out *after* they were written, pinned to exact file anchors); the review is pure disjoint reading. Both were reviewed against disk — the runbook's cross-links and env table checked, the review's every finding re-derived before applying (three real, the rest correctly cleared).

### The keeper this session: the drill can only ever pass, so it can never surface the one defect that would destroy a production DB — the adversarial *reader* found what the green *run* structurally couldn't

The drill restores into a **fresh, empty** target, so `pg_restore` never fails partway — it drops nothing (there is nothing to drop) and loads cleanly every time. That is precisely why a green drill is *not* proof the restore is safe: the destructive path — `--clean` auto-committing its DROPs of every domain table, *then* hitting an error (a truncated dump, a missing role, lock contention with a live Core) and stopping with **no rollback**, leaving a half-dropped production DB — only bites an in-place forced restore whose stream or roles or locks fail. The happy-path drill walks none of that. Nine green assertions said "backup works"; the footgun sat one `pg_restore` flag away and the run could not see it. The parallel adversarial agent, *reading* the script rather than running it, flagged it CRITICAL: no `--single-transaction`. One flag turns the whole DROP+CREATE+COPY into a single transaction — any failure rolls the target back to exactly its pre-restore state, and it reads a `-Fc` archive fine from the non-seekable `mc cat` stdin (only `-j` parallelism can't). The same review caught the S3 credential sitting in `docker run` argv (name-only `-e` fixes it) and a truncated-object window (atomic publish closes it). Keeper shape, echoing every prior slice and doubling down on part A's lesson: the green thing hides the thing that matters, and the reading — here, *one agent reading code the running drill could never exercise* — is what surfaces it. (The secondary keeper is the ninth-straight false-FAIL: run one of the drill reported the two RLS reads as `SET1`/`SET0` — not a restore bug but my assertion swallowing psql's `SET` command tag; the reads were the correct `1`/`0` all along. `tail -n1` fixes the harness, not the system.)

### Verification (rule 4)

- **The drill is green, live, on the dev host under `sg docker`.** Three runs tell the whole story: run 1 surfaced the `SET`-tag false-FAIL (system correct, assertion wrong); run 2 was a clean **RESTORE DRILL: PASS** (exit 0) once the assertion read the last line; run 3 was a clean PASS again *after* the security hardening (single-transaction, atomic publish, name-only creds) — confirming the fixes didn't regress the round-trip. Every run: source `_sqlx_migrations=12 messages=1`; after destroy+restore, `_sqlx_migrations=12`, `messages=1`, `opn_app` world-set read `=1`, cross-world read `=0`, Core Healthy on the restored DB.
- **The failure domain is genuinely exercised**, not asserted: step 4 drops only `${PROJECT}_pgdata` and step 4's own `mc ls` re-lists the dump *after* the drop, proving the MinIO-resident backup outlived the DB.
- **No Rust / Cargo / contracts / justfile changed** — nothing to `fmt`/`clippy`/`cargo test`, and the `contracts-drift` gate is untouched. `bash -n` clean on all five scripts. The `.dockerignore` change was proved harmless to the build: run 3 rebuilt the image with the new context and recompiled **0** crates (cache intact) and Core still booted Healthy — the offline build is unaffected by excluding `deploy/`.

### Exit criteria status (Sprint 11 — updated by part B slice 2)

| Criterion (roadmap Sprint 11) | Status |
|---|---|
| 1. Coolify deployment | **MACHINERY CLOSED (build)** — slice 1. Console deploy + LE cert operator. |
| 2. Partition automation (`pg_cron`) | **OPEN — held** (still wants Sprint 10's soak to validate partitioning before the tested janitor stopgap is deleted). |
| 3. Backups + restore drill | **MACHINERY + DRILL CLOSED** — the four scripts built and the **restore drill run live and green** on the dev host (round-trip + RLS + Core-serves-restored all asserted). Wiring the schedule onto Coolify, pointing the target off-box, and the operator's own periodic drill are operator. |
| 4. Alerts | **CLOSED (build)** — part A. |
| 5. Runbooks | **6 of 6** — `restore-from-backup.md` added this slice; all six now exist. "Commands executed once" for the infra runbooks stays operator. |
| 6. Contracts publish | **PIPELINE CLOSED (build)** — slice 1. A real publish needs the operator `NPM_TOKEN`. |
| 7. Release ritual documented | **CLOSED (doc)** — part A. |
| 8. Security pass | **secrets-in-logs audit + TLS/HSTS** closed slice 1; this slice also kept the new backup creds out of argv (name-only `-e`). `cargo deny` green since Sprint 9. |

### Reflection

- **Parallel juniors cross-checked the main thread again, and the dividend was the same as part A's: an independent reader disproved a premise a single writer would have shipped.** The green drill *was* the writer's confidence; the adversarial reader dismantled it. Worth repeating a third time: give parallel agents the reading even when only the main thread does the writing — the reading is where the load-bearing catch lives.
- **The whole slice is ~250 lines of bash and one runbook; the risk concentrated in two places** — is the restore actually recoverable, and is it *safe* when it fails — and both were closed by one live drill plus one adversarial read, not by more code. The §15 / ADR-1 verification budget keeps buying exactly this shape.
- **The drill is the deliverable, not the proof of the deliverable.** Item 3's text says an untested backup is a wish; a slice that shipped the scripts and deferred the drill would have shipped the wish. Running it — and modelling the real failure domain so the backup genuinely outlives the DB — is the item.

### Not committed / next session

- **Part B slice 2 is complete and green but untracked**, on top of untracked slice 1 + part A, on committed 0–10. New files: `opn-core/deploy/backup/{lib,pg-backup,pg-restore,media-mirror,restore-drill}.sh`, `docs/runbooks/restore-from-backup.md`; modified `opn-core/.dockerignore`. **No Rust / Cargo / justfile / contracts-wire change; drift gate untouched.** The operator's call to land, as every slice.
- **Next (buildable here):** Sprint 11 is nearly out of dev-host-buildable frontier. What remains is **item 2 (`pg_cron`)**, still deliberately **HELD** until Sprint 10's soak validates partitioning — deleting the tested janitor stopgap before then is the one part-B item that wants the perf runs first. Everything else in Sprint 11 is now operator: the Coolify console deploy + LE cert, a real off-box backup S3 target + the scheduled backup task, the operator's own periodic restore drill, and a real `@opn/contracts` publish with `NPM_TOKEN`.
- **Carried (unchanged):** the latent `Config`-`Debug` redaction (security-pass.md §1), the two still-unsampled gauges (`opn_sendq_depth`, `opn_connections`), and — the real gate — **all of Sprint 10's operator/pinned-hardware perf + 24 h soak runs**, which remain what stands between here and a real v1.0 tag.

---

## 2026-07-20 — Sprint 11 **part B**, slice 1 (the "deploy + publish" artifacts that need neither the perf numbers nor a live cloud host but DO need real Docker to trust: a production image `Dockerfile`, the Coolify/Traefik `docker-compose.prod.yml` with TLS/HSTS/sticky-WS and a first-boot `opn_app` password pre-seed, the `@opn/contracts` npm publish-on-tag pipeline, the deploy/rollback + security-pass runbooks, and the secrets-in-logs audit): built, **verified live in a container** (the production image builds offline, boots as `opn_app` with an operator-set password, `/healthz` → `{contracts_version, core_version, status:ok}`, janitor `message_partition`/`ledger_reconcile`/`media_verify` all `ok`, zero permission-denied); backups + the restore drill (item 3), `pg_cron` partition automation (item 2, still held for the soak), and everything needing the live Coolify console / a real cert deferred to a later slice

Part A shipped the release-engineering artifacts provable *without* Docker (runtime version reporting, the alerts, the `admin unfreeze` CLI, four docs). This slice is the next buildable frontier, and it opens on the same honest ordering call every Sprint-10/11 slice has: the roadmap makes 9–11 serial, but the *only* thing blocking is operator runs (Sprint 10's perf soak, Coolify's console, a real LE cert) — not code. What a dev host **with Docker** (`sg docker`, per the machine notes) can now prove is a materially stronger claim than part A could make: that the image actually builds and the stack actually boots. So this slice builds exactly the deploy machinery whose correctness lives in a real image build + a real Postgres init sequence, and marks the cloud-console/cert/soak-gated remainder as later. The one genuinely read-heavy junior task (the secrets-in-logs audit) fanned out to a parallel read-only agent; the npm pipeline would have too, but the agent launch was blocked by the auto-mode classifier, so it stayed inline (three small files). The load-bearing pieces — the Dockerfile, the compose role/TLS wiring, and the `initdb` pre-seed the live boot proved necessary — stayed on the main thread where the reading is load-bearing.

### What exists now

- **`opn-core/Dockerfile`** — two-stage: `rust:1.97.0-bookworm` builder (pinned to `rust-toolchain.toml`) → `debian:bookworm-slim` runtime carrying only the binary + `ca-certificates` + `curl`, run as a non-root uid-10001 user. It is a **pure offline build**: the crate has **zero** compile-time `query!` macros (all 596 DB calls are runtime `sqlx::query(...).bind(...)`), and `sqlx::migrate!` embeds the SQL into the binary, so the build needs no database, no `.sqlx` cache, and the runtime image needs no migrations dir. Verifying that query style *before* writing the Dockerfile is why it doesn't cargo-cult a build-time DB from other sqlx projects.
- **`opn-core/.dockerignore`** — keeps `target/`, `perf/results/`, logs, and the fuzz corpus out of the build context.
- **`opn-core/docker-compose.prod.yml`** — the Coolify stack: `core` (built from the Dockerfile) + Postgres 16 (tuned `shared_buffers=1GB`) + Redis + MinIO (+ one-shot `createbucket`) + coturn. Every credential is a `${VAR}` from Coolify's secret store — no password committed. The CDR-3 role split is wired (`OPN_MIGRATE_DATABASE_URL`=owner for migrations, `DATABASE_URL`=`opn_app` for RLS-enforced serving). Metrics are **internal-only** (no host port, no Traefik router); in fact **no store publishes a host port** — the only host-facing surface is Traefik→`core:8080`. Traefik labels: HTTPS router, LE `certresolver`, `tls.options=default@file` (min TLS 1.2), HSTS middleware (`max-age=31536000`, includeSubdomains, forceSTSHeader), and a sticky cookie for WS (cosmetic at one replica, configured now so replica-2 is a scale action, §7.1).
- **`opn-core/deploy/postgres/initdb/10-app-role-password.sh`** — the keeper (below). On a **fresh** data dir only, it pre-creates `opn_app` with the operator's `OPN_DB_APP_PASSWORD` *before* Core's migration `0001` runs; `0001`'s `IF NOT EXISTS` guard (0001:41) then skips its dev-`'opn'` seed but still applies its GRANTs. The least-privilege runtime role has a real password from first boot with no manual `ALTER ROLE` dance. `%L` + psql `:'var'` quoting make the password injection-safe.
- **`.github/workflows/contracts-publish.yml` + `crates/contracts/{package.json,.npmignore}`** — `@opn/contracts` npm publish on tag `v*`. The job: regenerate bindings → **enforce the drift gate in-job** (a tag on stale bindings must not publish) → version single-sourced from the crate's `Cargo.toml` (== `CONTRACTS_VERSION`) with a **tag-must-match hard-fail** → generate a barrel `index.ts` **at publish time only, after the gate** (so it never pollutes the committed `bindings/` drift check) → `npm publish` (needs the operator-provisioned `NPM_TOKEN` secret).
- **`docs/runbooks/deploy-rollback.md`** — required-env table, the one-time Traefik `default@file` tls-options block, first-deploy (the `initdb` pre-seed = no manual `ALTER`), update, rollback with the **forward-only-migration caveat** (binary rollback is safe only within a migration version — a rolled-back binary against a newer schema breaks), and verify-by-`/healthz`-version.
- **`docs/runbooks/security-pass.md`** — the secrets-in-logs audit result (clean; the `file:line` evidence, the latent `Config`-`Debug` risk, and the re-run greps), the secrets-not-in-repo posture, the TLS/HSTS summary, and the `cargo deny` gate.

### Decisions closed during implementation (roadmap deviations, all ponytail)

1. **Build part B's Docker-verifiable subset now; defer the cloud-only + soak-gated items.** Same ship-machinery-here / runs-stay-operator split as part A and all of Sprint 10 — but this slice's frontier is specifically what a dev host *with Docker* can prove: an image build and a first-boot Postgres init sequence. The Coolify console deploy, a real LE cert handshake, and deleting the janitor's partition stopgap (item 2, wants the soak first) stay deferred.
2. **An `initdb` pre-seed instead of a documented manual `ALTER` — the keeper (below).**
3. **Pure offline Dockerfile — no build-time DB, no `.sqlx`.** Read the query style first (0 `query!` macros) so the build is a plain `cargo build --release`. A Dockerfile that spun up Postgres at build time would be cargo-culted from compile-time-checked sqlx projects this repo deliberately isn't.
4. **No manifest-first dependency-cache layer in the Dockerfile.** The image builds on release tags, not per-commit, and a workspace makes the dummy-crate trick fiddly. Named the upgrade path (`cargo-chef`) in a `ponytail:` comment rather than paying its complexity for a rarely-built image.
5. **npm version single-sourced from the crate, with a tag-mismatch hard-fail.** The crate `Cargo.toml` version is already `CONTRACTS_VERSION` (part A); making the npm package read the same number and refusing to publish when the git tag disagrees means the crate, the running `/healthz`/link version, and the npm package can never drift apart.
6. **Barrel `index.ts` generated at publish time only, after the drift check.** Committing it would break the `contracts-drift` gate (a `git diff` on `bindings/`); generating it in-CI after the gate keeps both the `import { Cmd } from '@opn/contracts'` ergonomics and a green gate. Proved the loop locally (37 exports) and confirmed `bindings/` stays git-clean.
7. **Metrics + every store internal-only in the prod compose (no host ports).** The only host-facing surface is Traefik→`core:8080`. That is what "OPN_METRICS_BIND on the internal interface only" (item 1) actually means, and it shrinks the attack surface to one port.
8. **Deploy/rollback runbook written on the main thread, not delegated.** It is welded to my exact compose choices (`tls.options=default@file`, the `initdb` ordering, the forward-only rollback caveat); a junior would have had to reverse-engineer intent from the file. The genuinely-delegable read-heavy job (the secrets audit) went to a parallel agent; the npm pipeline would have too but the classifier blocked that agent launch, so it stayed inline (3 small files) rather than fought.

### The keeper this session: the faithful "production" compose would crash-loop on its very first deploy — reading the migration another sprint wrote is what surfaced it, and a fresh isolated volume is what proved the fix

The reflexive production compose uses `DATABASE_URL=opn_app:${OPN_DB_APP_PASSWORD}` — the obvious, faithful thing. It is wrong, and only reading migration `0001` before trusting the boot shows why. `0001` creates `opn_app` with a **hardcoded dev password `'opn'`** and a comment saying production must `ALTER ROLE` it out of band — but Core connects as `opn_app` *immediately* after migrating (`main.rs`: migrate as owner → close that pool → open the serve pool as `opn_app` → serve), so there is **no window** for a manual `ALTER` between migrate and serve; a first deploy with any real `OPN_DB_APP_PASSWORD` would crash-loop on a password mismatch before it ever served. The naive fixes are all worse: ship `opn_app:opn` in a file literally named `prod` (a weak committed password on the runtime role), or document a racey post-deploy `ALTER` squeezed into Core's restart backoff. The lazy-correct move is a Postgres `initdb` script that pre-creates `opn_app` with the operator password *before* Core migrates, leaning on `0001`'s own `IF NOT EXISTS` guard to make the pre-seed a no-op-then-GRANT — ~15 lines, zero change to tested migration code. And the proof that mattered was not "`docker compose config` validates" — it was booting the **real image on a fresh, isolated volume**. (The dev and prod compose share a `pgdata` volume name under the same default project, so a colocated test could silently reuse an old volume where `initdb` never re-runs and read a stale-volume false pass; I namespaced the test to a throwaway project to *guarantee* the fresh-init path ran.) The live boot then showed `opn_app` authenticating with the operator password `testapppw` and **not** the dev `'opn'`, `/healthz` green, and — the decisive one, echoing last session — the `SECURITY DEFINER` `message_partition` janitor task running `ok` as `opn_app` in the container. Keeper shape yet again: the faithful build looks done and hides the thing that actually matters (*can this stack boot the first time?*), and the reading — here, a migration a different sprint authored — is what surfaces it.

### Verification (rule 4)

- **No Rust changed this slice** — nothing to `fmt`/`clippy`/`cargo test`. The `contracts-drift` gate is untouched: `package.json`/`.npmignore` live at `crates/contracts/` (not in `bindings/`), and the publish-time barrel was proven to leave `bindings/` git-clean.
- **The production image builds:** `docker build -f Dockerfile .` → exit 0, fully offline (no DB).
- **End-to-end live boot of `docker-compose.prod.yml` on a fresh isolated volume:** `initdb` pre-seeded `opn_app` (`rolsuper=f, rolbypassrls=f, rolcanlogin=t, rolcreatedb=f` — matches `0001`:42-48) and it authenticated with the **operator** password, not the dev seed; Core built + booted **Healthy**; `/healthz` → `{"contracts_version":"0.1.0","core_version":"0.1.0","status":"ok"}`; `/metrics` (internal port, via `exec`) showed janitor `message_partition`/`ledger_reconcile`/`media_verify` all `outcome="ok"` — the `SECURITY DEFINER` DDL path works as `opn_app` in the image; **zero** `permission denied`/`ERROR`/`panic` in the Core log. Verify stack torn down `-v`.
- **npm pipeline:** `package.json` + the workflow YAML both parse; the barrel loop against the real `bindings/` produced 37 exports and left `bindings/` clean (generated `index.ts` deleted). `docker compose config` validates the full `${VAR}` interpolation.

### Exit criteria status (Sprint 11 — updated by part B slice 1)

| Criterion (roadmap Sprint 11) | Status |
|---|---|
| 1. Coolify deployment (prod compose, Traefik, healthz-gated) | **MACHINERY CLOSED (build)** — `Dockerfile` + `docker-compose.prod.yml` + `initdb` pre-seed built and **container-verified** (image builds, boots as `opn_app`, healthz green, TLS/HSTS/sticky labels wired). The actual Coolify-console deploy + LE cert is operator. |
| 2. Partition automation (`pg_cron`) | **OPEN — held** (deleting the tested janitor stopgap wants Sprint 10's soak to validate partitioning first). |
| 3. Backups + restore drill | **OPEN — next slice** (scripts buildable here; the drill needs a running stack). |
| 4. Alerts | **CLOSED (build)** — part A. |
| 5. Runbooks | **5 of 6** — `deploy-rollback.md` added this slice (+ `security-pass.md`); only `restore-from-backup` remains, deferred with backups. |
| 6. Contracts publish | **PIPELINE CLOSED (build)** — npm publish-on-tag workflow + `package.json`, version single-sourced + tag-guarded, barrel out of the gate. A real publish needs the operator `NPM_TOKEN`. |
| 7. Release ritual documented | **CLOSED (doc)** — part A. |
| 8. Security pass | **secrets-in-logs audit CLOSED (clean) + TLS/HSTS wired + documented**; `cargo deny` already green (Sprint 9). A live cert/HSTS handshake test is operator. |

### Reflection

- **Docker on the dev host moved the verifiable frontier.** Part A could only prove the non-Docker artifacts; with `sg docker`, this slice proved the image *builds* and the stack *boots* as the least-privilege role — a materially stronger claim than "the YAML validates." The honest split is no longer build-vs-runs but *build-and-container-boot-here* vs *cloud-console/cert/soak-there*.
- **The keeper was, an Nth time, more reading and one careful live test — not more code.** The `initdb` script is ~15 lines; the work was reading `0001` to see the first-boot trap and namespacing the test volume so the proof was real, not a stale-volume false pass. The §15 / ADR-1 budget keeps buying exactly this.
- **Laziness held the line where it counted.** No dep-cache cleverness and no build-time DB in the Dockerfile, a publish-time (not committed) npm barrel, no host ports on any store — each simplification named its ceiling in a comment rather than smuggling complexity back as prose.

### Not committed / next session

- **Part B slice 1 is complete and green but untracked**, on top of untracked part A, on committed 0–10. New files: `opn-core/{Dockerfile,.dockerignore,docker-compose.prod.yml}`, `opn-core/deploy/postgres/initdb/10-app-role-password.sh`, `opn-core/crates/contracts/{package.json,.npmignore}`, `.github/workflows/contracts-publish.yml`, `docs/runbooks/{deploy-rollback,security-pass}.md`. **No Rust / Cargo / justfile / contracts-wire change; drift gate untouched.** The operator's call to land, as every slice.
- **Next (buildable here): Sprint 11 item 3 — backups.** `pg_dump`-to-S3 + `mc mirror` scripts + the `restore-from-backup` runbook (the restore **drill** is operator — it needs a running stack). Then the last part-B doc polish. **HOLD** item 2 (`pg_cron`) until Sprint 10's soak has validated partitioning — deleting the tested janitor stopgap before then is the one part-B item that wants the perf runs first.
- **Carried:** the latent `Config`-`Debug` redaction (security-pass.md §1), the two still-unsampled gauges (`opn_sendq_depth`, `opn_connections`) from part A, and all of Sprint 10's operator/pinned-hardware perf runs remain the gate before a real v1.0 tag.

---

## 2026-07-20 — Sprint 11 **part A** (the release-engineering artifacts that need neither the perf numbers nor a live VM: contract-version reporting at runtime, the missing `admin unfreeze` CLI, the six §14 Prometheus alerts + the pool gauge one of them keys on, four grounded operational runbooks, and the release-ritual + contracts-semver docs): built, **verified live** (Core boots as `opn_app`, `/healthz` → `{contracts_version, core_version, status}`; `admin_unfreeze_clears_freeze`, `pool_in_use_tracks_checked_out`, `hello_ack_carries_contracts_version`, all healthz/link/contracts suites green; zero contracts drift); the Coolify deploy, the npm publish pipeline, pg_cron partition automation, backups/restore-drill, and the two infra-dependent runbooks deferred to **part B**

This is the first Sprint 11 slice, and it opens on an honest ordering call: the roadmap makes 9–11 strictly serial, but Sprint 10's entire remainder is operator/pinned-hardware *runs* (profiling, the fix loop, the tightened smoke, the 24 h soak) that this dev machine cannot do. So the only buildable frontier here is the subset of Sprint 11 that depends on Sprint 10's *code* (which exists) and not its *numbers* — deploy config, CI, docs, and version-reporting. Building that ahead of the operator's perf runs is the same "ship the machinery here, the runs stay operator" split every Sprint-10 slice used; nothing in this slice touches the tested runtime hot path except one additive metric sample. Two juniors' worth of the work (alerts + docs) fanned out to parallel opus subagents; the code and the one cross-agent finding stayed with the main thread.

### What exists now

- **`CONTRACTS_VERSION`** — a single embedded const (`env!("CARGO_PKG_VERSION")`, `contracts/src/lib.rs`), the source of truth for the wire-contract version. Surfaced live in two places (roadmap Sprint 11 item 6): the **`/healthz` JSON body** (`{status, contracts_version, core_version}`, on both the 200 and 503 path — `http/mod.rs`, return type moved from `&'static str` to `Json<Value>`) and the **`/link` hello ack payload** (`gateway/link.rs` `hello_ack()`), so a deploy or a triage confirms which contract is live without shelling in. Additive: the ack consumers only assert `ok`/`reply_to`, so the new payload broke nothing (9 link tests green).
- **`admin unfreeze --world <uuid> --account <uuid>`** (`admin.rs`) — the reconciliation-recovery CLI the roadmap named a Sprint 7 exit criterion and migration `0010_ledger.sql:15` references *by name*, but which **never actually shipped** (only `create-tenant` existed). Runs as the owner role (`OPN_MIGRATE_DATABASE_URL`, RLS-bypassing), scopes the world explicitly, and only ever thaws a *currently-frozen* account (a no-op reports, never silently "succeeds"). The DB half is the extracted `pub fn unfreeze_account(pool, world, account) -> u64`, driven by a `#[sqlx::test]`.
- **Six Prometheus alert rules** (`opn-core/deploy/prometheus/alerts.yml` + README, roadmap item 4) — the four §14 alerts on Core's own metrics (`HighCommandLatencyP99` p99>25 ms/5 m, `DurableQueueCloses`, `PgPoolExhaustion`, `JanitorFailures`) plus two ops-level (`HealthzDown` via blackbox, `DiskAlmostFull` via node_exporter, both flagged as needing exporters Core doesn't ship). PromQL and metric names code-verified.
- **A now-live `opn_pg_pool_in_use` gauge** (`janitor.rs`) — sampled every 30 s from `pool_in_use(&pg)` = `size() - num_idle()`. Before this the gauge was `describe`d and pinned at 0 in `observe.rs`, so the `PgPoolExhaustion` alert above **could never fire** — the session's keeper (below).
- **Four operational runbooks** (`docs/runbooks/`, item 5) grounded in shipped code with `file:line` citations: `incident-triage.md` (healthz→metrics→logs-by-span), `frozen-account.md` (the `ledger_reconcile` freeze + the new `admin unfreeze` recovery), `replica-2-scale-up.md` (`OPN_REPLICAS>1` → Redis fan-out listener), `jwt-rotation.md` (single-secret swap now; dual-secret overlap marked *planned*). The two infra-dependent runbooks (deploy/rollback, restore-from-backup) are deferred to part B with the mechanisms they document — writing them now would be fiction.
- **`docs/releasing.md`** (item 7, release ritual + standing cadence, auto-vs-operator marked, honest gaps flagged: the weekly 1 h mini-soak is *not* CI-wired, "pages on red" is GitHub-notification-only) and **`docs/contracts-semver.md`** (item 6, additive-only-within-major, closed `ErrCode`, the `contracts-drift` gate, the live `CONTRACTS_VERSION`, npm publish marked *planned*).

### Decisions closed during implementation (roadmap deviations, all ponytail)

1. **Start Sprint 11's buildable subset rather than idle on the serial rule.** Sprint 10's gates are unmet but only by operator runs this machine can't perform; the disciplined move is to build the Sprint 11 artifacts that need Sprint 10's *code* (present) not its *numbers*, and mark everything perf/VM-dependent as part B. Same split the whole of Sprint 10 used. The one runtime change (the pool gauge) is an additive metric sample, not a hot-path behavior change.
2. **Built `admin unfreeze` this slice instead of documenting manual SQL.** The frozen-account runbook needs a recovery command; the roadmap/migration promised one; it didn't exist. A production runbook telling operators to hand-run `UPDATE accounts SET frozen_at=NULL` on the money table is a foot-gun. ~30 lines mirroring `create-tenant` closes a real Sprint 7 gap and makes the runbook honest. Manual SQL survives only as a warned break-glass.
3. **Split `unfreeze_account` out of the CLI for a `#[sqlx::test]`.** The CLI builds its own owner pool from env; the DB logic is the testable part. Extracting it lets the test reuse the ledger fixture's freeze helper and assert the full round-trip (frozen→conflict→thaw→sends→second-thaw-is-no-op) as the owner role, exactly the CLI's path.
4. **Wired only the *one* unsampled gauge that gates a shipped alert.** `opn_pg_pool_in_use`, `opn_sendq_depth`, and `opn_connections` were all `describe`-only at 0 (an explicit Sprint 0 "define names now, sample later" decision). I sampled only `pg_pool_in_use`, because it is the one a *shipped page-severity alert* depends on. Wiring the other two is real work touching the gateway/writer paths for no alert I'm shipping — left documented, not done. A principled line ("wire what a shipped alert needs"), not an arbitrary one.
5. **Sampled the pool gauge on the janitor's 30 s tick, not a new task or a hot-path hook.** The janitor is the one always-running loop with `&state` and a steady tick; a per-command hook would tax the hot path, a dedicated spawn is a whole task for one line (YAGNI). 30 s instantaneous granularity is coarse but suits a "sustained 1 min" exhaustion alert — documented inline and in the runbook.
6. **Four runbooks, not six.** Only wrote the runbooks whose subject already ships; deploy/rollback and restore-from-backup depend on the Coolify/backups work (part B) and would be fiction now. Reconciled the two docs that referenced them (releasing.md step 5, incident-triage's pool note) so nothing points at a file that doesn't exist.
7. **Fanned the docs/config out to three parallel opus juniors; kept the code + the finding.** Alerts, runbooks, and the release/semver docs are independent, disjoint-path, low-code-risk — ideal junior work, each pinned to exact metric names / CLI spellings / file anchors so they couldn't guess wrong. The version-reporting code, the `admin unfreeze` build, and the cross-agent pool-gauge finding stayed on the main thread where the reading is load-bearing. Reviewed every junior file against disk (PromQL, CI job names, the CONTRACTS_VERSION claims) before trusting.

### The keeper this session: two parallel agents cross-checked each other and caught a page-severity alert that could never fire — and the honest proof was the deterministic test, not the live load that couldn't observe it

The alerts junior shipped, correctly per its brief, a `PgPoolExhaustion` page alert keyed on `opn_pg_pool_in_use >= 20`. Independently, the runbooks junior — reading `observe.rs` to write the triage table — flagged that same gauge as *registered but never sampled, reads 0*. The convergence is the catch: a §14-mandated, page-severity alert was keyed on a gauge hardwired to 0, so it would **never fire** — a false safety net on a money/comms backend, which is strictly worse than no alert. Neither agent alone surfaced it (the alerts agent trusted the metric existed; the runbooks agent wasn't writing alerts); the main thread reconciling their outputs did. The fix is one sampled line, but the *verification* is where the session earned its budget: the reflexive "boot Core and watch it go >0 under load" came back **flat 0 across 40 s of 120-way concurrency** — not a bug, but a measurement artifact (a sub-ms `SELECT 1` almost never coincides with a once-per-30 s instantaneous sample, and undici caps client concurrency). A skeptic can't tell a live-idle-0 from the old hardwired-0 in that scrape, so the load test *proves nothing*. The honest close was to stop trying to observe it and instead prove it deterministically: extract `pool_in_use`, then a `#[sqlx::test]` that holds two acquired connections and asserts the value rises ≥ 2 above baseline. That test is the permanent guard that the gauge tracks real usage; the alert now has something true to fire on. Keeper shape, an eighth-plus time: the naive move (edit the DSN / boot-and-eyeball) looks done and hides the thing that matters (is the least-privilege role complete / *can this alert actually fire*), and the reading — here, one agent reading the code another agent didn't — is what surfaces it.

### Verification (rule 4)

- `cargo fmt --check` clean; **`cargo clippy --all-targets -- -D warnings` clean** across the workspace after every edit (compiles all tests). No new `unwrap` outside the permitted sites.
- **Unit/integration tests green:** `hello_ack_carries_contracts_version` (link ack payload), `healthz_body_reports_contracts_version` + the two pre-existing healthz tests (return-type change didn't break the status-only assertions), `admin_unfreeze_clears_freeze` (owner-role freeze→conflict→thaw→send→no-op), `pool_in_use_tracks_checked_out` (held connections raise the gauge), all **9 link** tests (ack change backward-compatible), all **32 contracts** tests.
- **Drift gate untouched** — `CONTRACTS_VERSION` is a `const`, not a `#[derive(TS)]` type; re-running `export_ts` produced no `bindings/` diff. The hello ack change reuses existing `ServerMsg` types.
- **Live boot as `opn_app`** (dev compose up; Core booted with the split DSNs): `/healthz` returned `{"contracts_version":"0.1.0","core_version":"0.1.0","status":"ok"}`; `/metrics` served `opn_pg_pool_in_use` (a real sample, no longer describe-only) and `opn_janitor_runs_total{task="expired_sessions",outcome="ok"}` confirming the tick loop — with the pool-gauge line — ran.
- The inconclusive-by-design load run is recorded above (40 s, 120-way, max pool 0) precisely so the next reader knows *why* the deterministic test exists and doesn't "improve" it into a flaky live check.

### Exit criteria status (Sprint 11 — part A slice; part B = the deploy/CI/backup machinery + the runs)

| Criterion (roadmap Sprint 11) | Status |
|---|---|
| 1. Coolify deployment (prod compose, Traefik, healthz-gated rollout) | **OPEN — part B** (needs the host/console; the deploy/rollback runbook ships with it). |
| 2. Partition automation (`pg_cron`, delete the janitor stopgap) | **OPEN — part B** (deliberately held: it rips out tested runtime code and wants Sprint 10's soak to validate partitioning first). |
| 3. Backups + restore drill | **OPEN — part B** (scripts + the drill need a running stack). |
| 4. Alerts (four §14 + two ops) | **CLOSED (build)** — six rules; `PgPoolExhaustion`'s gauge wired live + unit-proven. A real page test-fire is operator. |
| 5. Runbooks | **PARTIAL** — 4 of 6 written (the ones whose subject ships); deploy/rollback + restore deferred to part B. "Commands executed once" is operator. |
| 6. Contracts publish | **PARTIAL** — `CONTRACTS_VERSION` embedded + surfaced (/healthz + link ack) + semver doc; npm publish-on-tag deferred to part B. |
| 7. Release ritual documented | **CLOSED (doc)** — `releasing.md`, honest gaps flagged. Running the ritual is operator. |
| 8. Security pass | **PARTIAL** — `cargo deny` already green (Sprint 9); JWT-rotation runbook written; secrets-in-logs grep audit + Traefik TLS deferred to part B. |

### Reflection

- **The parallelism paid a dividend beyond speed: it cross-checked itself.** Three juniors ran while the main thread wrote code; the value wasn't just wall-clock — the runbooks agent, reading `observe.rs` for an unrelated purpose, disproved a premise the alerts agent (and I) had taken for granted. Independent readers of the same codebase catch what a single writer's confirmation bias misses. Worth repeating the shape: give parallel agents overlapping *reading* even when their *writing* is disjoint.
- **Laziness was, again, more reading and one careful test — not less code.** The whole slice is ~195 lines of Rust plus docs; the risk concentrated in two places (the version-reporting return-type change, and whether a shipped alert can fire), and both were closed by reading + a deterministic check rather than by shipping and hoping. The §15/ADR-1 budget keeps buying exactly this.
- **The honest deferral list is the deliverable's other half.** Part A ships what's provable on a dev machine and names — precisely, per item — what needs the perf host, the Coolify console, or a running stack. A release-engineering slice that pretended those were done would be the one failure mode this sprint exists to prevent.

### Not committed / next session

- **Sprint 11 part A is complete and green but untracked**, on top of committed 0–10. It modifies `contracts/src/lib.rs`, `core/src/{admin,janitor}.rs`, `core/src/gateway/link.rs`, `core/src/http/mod.rs`, `core/tests/{healthz,ledger}.rs`, and adds `docs/runbooks/` (4 files), `docs/{releasing,contracts-semver}.md`, and `opn-core/deploy/prometheus/{alerts.yml,README.md}`. No Cargo/justfile/contracts-wire change; drift gate untouched. The operator's call to land, as every slice.
- **Next — Sprint 11 part B (buildable here):** the Coolify production compose + Traefik labels (item 1) and its deploy/rollback runbook; the npm publish-on-tag workflow + `contracts_version` in the CI (item 6); the backups scripts + the restore-from-backup runbook (item 3, drill is operator); the secrets-in-logs grep audit + Traefik TLS notes (item 8). **Hold** `pg_cron` partition automation (item 2) until Sprint 10's soak has validated partitioning — deleting the tested janitor stopgap before then is the one part-B item that wants the perf runs first.
- **Carried, non-code:** the other two unsampled gauges (`opn_sendq_depth`, `opn_connections`) remain at 0 — sampling them touches the gateway/writer paths and gates no shipped alert, so it's a tracked follow-up, not a blocker. And all of Sprint 10's operator/pinned-hardware runs remain the gate before a real v1.0 tag.

---

## 2026-07-20 — Sprint 10 **part B2 code close-out** (the two flagged carry-overs: perf-smoke's runtime Core moved off the migrate **superuser** onto the least-privilege `opn_app` role, and the seed loop made bounded-concurrent so the 100/3000-conn heavy runs don't serialize their mint): built, **verified live as `opn_app`** (8-conn/4-pair dev-host run: `sends=72 → recvs=144`, `delivery_count=72`, `seq_gaps=0`, `durable_closes=0`, `errors=0`, exit 0 — **and every janitor task 3×/`ok` including `message_partition`, `ledger_reconcile`, `media_verify`**); this closes the **last buildable code of Sprint 10** — everything that remains is operator/pinned-hardware runs

The prior three sessions each closed one B2 *driver mode* and ended on the same refrain: "all three driver modes done; what remains is not code." That refrain had one asterisk — two carry-overs the slices kept deferring as "non-blocking," both flagged since part A/B1: (1) `perf-smoke.yml`'s runtime Core still ran as `opn_migrate`, and (2) the seed minted sessions one-at-a-time. Neither is a driver mode, so no slice owned them; but both are *code the release gate needs before the operator's heavy runs*, so they are the honest close-out of Sprint 10's build. This session does exactly those two, and nothing speculative beyond them — the profiling/fix loop, tightened thresholds, and 24 h soak are runs, not diffs, and stay the operator's.

### What exists now

- **`perf-smoke.yml`'s runtime Core serves as `opn_app`** (`.github/workflows/perf-smoke.yml`) — one env line: `DATABASE_URL` moved from `opn_migrate:opn` to `opn_app:opn`; `OPN_MIGRATE_DATABASE_URL` stays the owner role. Core runs startup migrations via the migrate URL (`main.rs:37`) then serves the pool via `DATABASE_URL` (`main.rs:51`), so the split is exactly what the two-URL config was built for (`config.rs` already documents `database_url` as "the non-BYPASSRLS `opn_app` role"). The nightly smoke now exercises the **RLS-enforced** path production actually runs — not the superuser path that silently bypasses `FORCE ROW LEVEL SECURITY`. A seven-line comment records why the runtime and migrate roles differ so the next reader doesn't "simplify" them back together.
- **Bounded-concurrent seed** (`crates/loadgen/src/main.rs`) — the sequential `for i in 0..conns { mint().await }` became `futures_util::stream::iter(0..conns).map(|i| async { mint(i) }).buffered(SEED_CONCURRENCY).try_collect()`. `buffered` (not `buffer_unordered`) is load-bearing: it yields in **input order**, so `sessions` stays index-aligned — the paired topology (conns `2i`/`2i+1`) and the group creator (index 0) both depend on that; an unordered collect would silently shuffle pairs and the live run would show broken delivery. `try_collect` short-circuits on the first mint error with the same `mint session {i}` context as before.
- **`SEED_CONCURRENCY = 20`** — held at the server's pool ceiling (`max_connections = 20`): higher just queues on `pg` acquire (3 s timeout), lower leaves the pool idle. A named const with the rationale inline, not a config field (YAGNI — one number, one call site).

### Decisions closed during implementation (roadmap deviations, all ponytail)

1. **Only `perf-smoke.yml`'s runtime role changed — not `ci.yml`'s or `nightly-verify.yml`'s.** Those two also set `DATABASE_URL=opn_migrate`, but there the URL drives `#[sqlx::test]`, which *creates* a fresh database per test and runs migrations — work `opn_app` (NOCREATEDB, not the table owner) cannot do. perf-smoke is different: it boots a long-running **server binary** that only ever serves domain traffic, which is precisely the least-privilege runtime role's job. The reflection flagged only perf-smoke for a reason; changing the test harnesses would break them.
2. **Verified the least-privilege role is *complete* before trusting the flip — the keeper (below).** The one-line DSN change is trivial to write and impossible to trust by inspection: the real question is whether `opn_app` can do *everything the runtime does*. Answered by reading the grant model (every domain table `GRANT … TO opn_app`, migrations 0003–0012) **and** the one runtime op that needs owner rights — the janitor's `message_partition` task, which `CREATE TABLE`s a partition. That reaches the owner through `ensure_message_partition`, a `SECURITY DEFINER` function owned by the migrate role with `GRANT EXECUTE … TO opn_app` (0005:96–118). So nothing at runtime needs the superuser — the design already bridged the one gap. Then *proved* it live, not just reasoned it.
3. **`buffered`, not `buffer_unordered`.** The seed's whole contract is index-aligned output; ordered concurrency is the correct primitive and no slower for this (all N futures still run ≤20-at-a-time). No sort, no `(i, result)` tagging — `buffered` gives ordering for free.
4. **No new unit test for the seed.** The order-preservation is a documented `futures` guarantee; a test asserting it would re-test the library (YAGNI applies to tests too). The permanent guard is the live pairing run: 4 pairs formed and delivered peer-to-peer (`delivery_count=72`, self-loopback excluded), which is impossible if `buffered` reordered — the pairing *is* the assertion.
5. **Did this solo, not via a junior.** The two diffs are ~20 lines total and their correctness is welded to the reading (the SECURITY-DEFINER bridge, the `buffered`-vs-unordered ordering trap, the ci.yml-scope distinction); the single verification is a serial live pipeline (compose up → boot → mint → run) that doesn't parallelize. A delegated one-line YAML edit would cost more coordination than it saves, and the risky part isn't the edit — it's knowing the flip is safe. Kept it whole.

### The keeper this session: running the runtime as the migrate **superuser** had been silently masking whether `opn_app`'s runtime privileges are complete — and the proof that mattered wasn't the loadgen PASS, it was the janitor metric

For nine sprints the perf smoke booted Core as `opn_migrate`, which `docker-compose` creates as `POSTGRES_USER` — a **superuser**, which bypasses RLS unconditionally, `FORCE` or not. So the nightly smoke never actually tested the role production runs as; it tested a role with strictly more power, which can hide two whole classes of latent breakage: a domain table missing its `opn_app` grant, and a runtime op that quietly needs owner rights. The reflex flip (edit the DSN, done) ships that risk into the next nightly and calls it fixed. The lazy-correct move is the opposite of less work here: read what the least-privilege role must be able to do *before* trusting the switch. The grant audit (0003–0012) covers the request path, but the subtle one is the janitor's `message_partition` task — the single runtime code path that issues DDL. Reading it showed the design already anticipated exactly this: a `SECURITY DEFINER` function bridges the owner gap, granted to `opn_app`. That is why the flip is safe — a fact invisible from the workflow file, visible only two files deep. And the confirmation that sealed it wasn't the loadgen summary (a green `sends/recvs` only proves the *request path* — the thing grants most obviously cover); it was `opn_janitor_runs_total{task="message_partition",outcome="ok"} 3` on the live `/metrics` — the one op that would have been the *first* to break under a real privilege gap, run three times, zero errors, as `opn_app`. The permanent guard is now the nightly itself: from here on the smoke fails if any runtime privilege is missing, because it finally runs as the role that has exactly the runtime privileges — no more, no less.

### Verification (rule 4)

- `cargo fmt -p opn-loadgen --check` clean; `cargo clippy -p opn-loadgen --all-targets -- -D warnings` clean — the seed rewrite adds two trait imports (`StreamExt`, `TryStreamExt`) and one const; no new `unwrap`, no core-crate change.
- **24 loadgen unit tests green** (unchanged — the seed's correctness is a live-run property, not a unit one, per decision 4).
- **Live end-to-end run as `opn_app`** (dev compose: Postgres 16 + Redis 7, debug Core booted with `DATABASE_URL=opn_app`, `OPN_MIGRATE_DATABASE_URL=opn_migrate`; throwaway 8-conn/4-pair/8 s scenario): **`connections=8`, `sends=72`, `recvs=144`** (2× fan-out: sender's own sub + peer), **`delivery_count=72`** (peer deliveries, self excluded — proves the 4 pairs formed correctly, i.e. the batched seed preserved order), **`seq_gaps=0`, `durable_closes=0`, `error_acks=0`, `errors=0`, `rate_limited=0`, exit 0, PASS**. Every request-path op (HTTP mint → WS auth → `channels.open_direct` → `channels.send` → fan-out) acked ok through `world_tx` as the NOSUPERUSER/NOBYPASSRLS role.
- **Janitor proof (the decisive one):** live `/metrics` showed **all twelve janitor tasks at `outcome="ok"` ×3, zero `err`** — critically `message_partition` (the `SECURITY DEFINER` `CREATE TABLE` path), `ledger_reconcile`, and `media_verify`, all as `opn_app`. Core log had zero `permission denied` / `ERROR` lines.
- Drift gate untouched — no contracts change; `SEED_CONCURRENCY` and the `buffered` seed are loadgen-internal, the DSN is CI env. No `.d.ts` moved.

### Exit criteria status (Sprint 10 — this closes the **buildable-code** work of the whole sprint; the remainder is operator/pinned-hardware runs)

| Criterion (roadmap Sprint 10) | Status |
|---|---|
| All five loadgen scenarios exist and are runnable | **CLOSED** — `design`/`soak10x`/`reconnect-storm`/`hot-channel`/`call-churn` (parts A + B2 slices 1–3). |
| Measurement discipline (JSON summary, `perf/results/`, soak-safe histogram) | **CLOSED** — parts A + B1. |
| Nightly smoke runs the production RLS path (item 7 carry-over, part A) | **CLOSED** — perf-smoke runtime Core now `opn_app`, live-proven complete (grants + SECURITY-DEFINER partition bridge). |
| Seed scales to soak/heavy connection counts (carry-over, B1) | **CLOSED** — bounded-concurrent `buffered` seed, order-preserving. |
| Profiling passes + fix/re-measure loop | **OPEN — operator** (pinned hardware, evidence-first per item 4). |
| Tighten nightly smoke thresholds to measured-p99 + 50 % (item 7) | **OPEN — operator** (needs the measured p99 from the pinned runs). |
| All six targets met, numbers committed to `perf/results/`; 24 h soak green | **OPEN — operator** (the runs; machinery all shipped). |

### Reflection

- **The build side of Sprint 10 is done.** Every diff the sprint calls for — the seq verifier (A), the soak-safe histogram (B1), the three driver modes (B2 slices 1–3), and now the two role/seed carry-overs — is written and live-verified. What's left is exclusively runs on pinned hardware: profile, fix-and-re-measure, tighten the smoke to the measured number, commit the six target numbers, and the 24 h 10× soak. No more loadgen or CI *code* stands between here and Sprint 10's gates.
- **The keeper pattern held an eighth time, and this was its purest form.** Every slice's keeper came from reading the real semantics before writing — out-of-order delivery, replay-before-ack, `member_add`'s missing cap, dead-sub accretion. This one: reading whether the least-privilege role is *complete* before flipping to it, and finding the SECURITY-DEFINER bridge that makes it safe. The tell each time is the same — the naive move looks done and hides the thing that matters.
- **Laziness was doing *more* reading, not less code.** The change is two lines of substance. The work was proving they're safe — which is exactly the budgeted verification the §15 / ADR-1 amendment buys. A one-line DSN flip shipped blind would have been the fast wrong version.

### Not committed / next session

- **This close-out is complete and green but untracked**, on top of committed 0–9 + Sprint 10 part A, and the untracked B1 + B2-slices-1/2/3 stack. It touches `.github/workflows/perf-smoke.yml` (the `DATABASE_URL` role + comment) and `crates/loadgen/src/main.rs` (the `buffered` seed, `SEED_CONCURRENCY`, two `futures_util::stream` imports). No Cargo/justfile/contracts change. The operator's call to land, as every slice.
- **Next: Sprint 10 is now entirely operator/pinned-hardware work — no loadgen or CI code remains.** In order: (1) pinned-hardware profiling (`pg_stat_statements` → `cargo flamegraph` → `tokio-console`, evidence-first per item 4); (2) the fix + re-measure loop (each fix its own commit with before/after numbers); (3) tighten the nightly smoke to measured-p99 + 50 % (item 7 — now that its role is correct, only the threshold number is left); (4) the heavy scenario runs (`design`/`hot-channel`/`reconnect-storm`/`call-churn`) with results committed to `perf/results/`; (5) the 24 h 10× `soak10x`. After Sprint 10's gates are met, Sprint 11 (release engineering) is the last sprint.
- **Carried, now CLOSED:** both non-blocking carry-overs from part A/B1 — the perf-smoke runtime role and the sequential seed — are done this session. No code-side carry-overs remain for Sprint 10.

---

## 2026-07-20 — Sprint 10 **part B2**, scenario slice 3 (`call-churn`: the loadgen grows a calls driver — N caller/callee pairs, one task per pair owning both sockets, each churning a full call lifecycle (`calls.start` → `calls.accept` → `calls.signal` both ways → both `calls.hangup`) at a per-pair rate, while one shared tenant `/link` consumer drains the `set_targets`/`clear` voice-target events): built, verified live (dev-host 4-pair run: `sends=32` completed calls, `set_targets=64`, `recvs=96` link events drained, `durable_closes=0`, `errors=0`, exit 0); this is the **last new driver mode** — the rest of B2 (100-conn run, profiling/fix loop, tightened smoke, 24 h soak) is operator/pinned-hardware work

Slices 1–2 grew the *message* driver (`reconnect-storm` = a new connection lifecycle on the same DM topology; `hot-channel` = a new group topology reusing the existing member half). Slice 3 is the first mode that drives a *different protocol entirely* — calls, not channel messages — so it does not fit `drive()` (channel-centric: seq trackers, send/read mix, resume). It is a wholly separate driver that the `run_scenario` calls-branch early-returns into, leaving the message path byte-identical. The reuse still ran deep: the call frames were already spoken by the `--link-drop` chaos checker (`linkdrop.rs`), so `connect_auth`/`connect_link`/`start_call`/`accept_call`/`link_url` became `pub(crate)` and the only genuinely new code is `calls.signal`/`calls.hangup` (one command + await-ack each) and the churn loop. This is roadmap item 1's `call-churn` bullet — "50 concurrent calls starting/ending at 1 Hz with signaling — exercises link + FSM under load" — verbatim.

### What exists now

- **A `callchurn.rs` driver + a `calls` scenario branch** — one task per caller/callee pair owns BOTH sockets and drives the lifecycle strictly sequentially (send → await ack → next), so a `calls.start`→`accept`→`signal`×2→`hangup`×2 round is 8 in-order round trips. 50 pairs run 50 lifecycles concurrently — the FSM-under-load shape (each pair hammers its own session's `FOR UPDATE` row locks; 50 sessions churn at once). `run_scenario` gained a `scenario.calls` branch that early-returns into `callchurn::run_calls`, so the entire message driver below it is unreached and unchanged for every other scenario.
- **One shared `/link` consumer** (`callchurn::drive_link`) — the tenant `/link` is last-writer-wins, so exactly one consumer, spawned first (during warmup, before the first accept emits a target) and draining until the run's read deadline. It counts `set_targets` (the non-vacuity proof the relay fired) and treats a `4409` as the link-under-load failure. This is the fan-out-under-load target: at 50 calls/s the link receives ~150 durable voice events/s and must never slow-consumer-close.
- **`set_targets` counter on `ConnStats`/`Summary`** (`driver.rs`, `main.rs`) — one new loadgen-internal field (the precedent of `seq_gaps`/`resume`/`reconnects`), summed in `merge`, emitted in the JSON line + (calls-only) human table, and gated by `assert_calls` (fail if `sends==0 || set_targets==0`). The `ack` histogram is reused for the start→active **call-setup** latency; `delivery` is empty by design (the parties subscribe to nothing).
- **Reused linkdrop call frames** — five helpers went `pub(crate)` with a note; no behavior change, and the 5 linkdrop tests + the `--link-drop` drill are untouched. `calls.signal`/`calls.hangup` are the only new frame helpers.
- **`crates/loadgen/scenarios/call-churn.json`** (junior) — the real 100-conn/50-pair spec: `calls: true`, `calls_per_sec: 1.0`, `warmup_secs: 15`, gates `assert_calls` + `assert_no_durable_closes`, `assert_ack_p99_ms` omitted (item 7 tightens it after the fix loop — same call as every B2 scenario). `total_msgs_per_sec` is present-but-unused in calls mode (required by the struct). Runs via `just perf call-churn` for free (scenario-parameterized recipe from B1; no justfile change — and no coturn: the FSM + signaling relay need no TURN, only a real WebRTC media leg would).
- **`perf/results/README.md`** (junior) — a `call-churn` scenario bullet after `hot-channel`.

### Decisions closed during implementation (roadmap deviations, all ponytail)

1. **The parties do NOT `sub call:<id>` — the keeper (below).** Signals are sent and ok-acked every round (the relay authz + publish path is exercised), but subscribing each party to every round's fresh `call:<id>` would leak dead topic subs into Core's registry (parties never unsub a call that has Ended: at 1 Hz for minutes that is hundreds of stale subs per connection) *and* add a per-round socket-drain burden — for zero load coverage the link doesn't already give. The signal fan-out is a 2-party topic and no load stressor; the link (1 consumer, every call) is the real fan-out target and IS subscribed.
2. **The callee learns `call_id` in-process, not via a notify ring sub.** Both sockets live in one task, so `start_call` returns the id directly to the `accept`. Core still *emits* the ring (`notify::route(class=ring)` runs on every start), it just has no local subscriber — the emit path is under load, the consume path (a device's notify sub) is a different scenario's concern. Wiring a real notify-ring consumer would need device-id + ring-payload plumbing for no FSM/link coverage gain.
3. **A wholly separate driver, early-returned — not a fifth `Pairing` variant.** `reconnect-storm`/`hot-channel` grew the message `drive()` because they *were* message topologies. Calls is a different protocol (no channel, no seq, no resume), so bolting it into `drive()` would fork every branch of a channel-centric loop. A `callchurn.rs` module the `run_scenario` calls-branch early-returns into is the smaller diff and keeps the message path byte-identical (proof: the 23 prior tests + every non-calls scenario are untouched).
4. **One task owns both pair sockets, driven sequentially.** The "concurrency" the scenario needs is 50 *pairs* hitting the FSM at once, not intra-pair parallelism — a caller acts, then the callee responds, which is the realistic call flow and needs no inter-task channel/coordination. Sequential send-await-ack also keeps both sockets drained (they receive only their own acks), so there is no backpressure bookkeeping at all.
5. **Both parties hang up; the last hangup ends the session.** Caller hangup → `Left` (session still `active` while the callee is `joined`), callee hangup → last → `Ended` → link `clear`. Ending the session frees both numbers so the next round's `start` is never rejected `busy`. Reading the FSM (`Joined --hangup--> Left`, session `--last_hangup--> Ended`) before writing the round is what made this deterministic rather than a flaky busy-race.
6. **Reuse `ack` for call-setup latency; add only `set_targets`.** The setup round-trip (start→active) is the natural thing the `ack` histogram measures for a call, and `set_targets` is the one genuinely new signal (the link relay firing) that no existing field carries. `delivery`/`resume`/`seq_gaps` stay 0 in calls mode — honest empties, not forced reuse.
7. **Delegated the two disjoint mechanical files to a parallel junior; kept the driver.** `call-churn.json` + the README bullet touch no `.rs` and were pinned to the exact new field names (`calls`/`calls_per_sec`/`assert_calls`, `assert_ack_p99_ms` omitted) so the junior couldn't guess them wrong — run in parallel while I did `callchurn.rs`, the linkdrop `pub(crate)` surgery, and the `Summary` wiring. Reviewed both against disk (parsed the JSON, confirmed the field spellings and the omitted gate) before trusting; clean again.

### The keeper this session: the faithful build would poison the very thing under test — subscribing the parties leaks dead registry subs, and the fan-out target is the link, not the call topic

The reflexive "faithful" call-churn subscribes both parties to `call:<id>` (so signals actually deliver) and rings the callee through a notify sub (so the callee learns the call the way a real dialer does). Both are wrong here, and only reading two things before writing shows why. First, the registry sub lifecycle: a party that subs `call:<id>` never unsubs it — the call Ends but the stale topic entry lives until the connection closes, so at 1 Hz over a minutes-long run each of 100 party connections accretes hundreds of dead subs in Core's `topics` map. A scenario whose job is to measure Core under load would be *inflating Core's own memory with test-artifact subscriptions* — measuring a pathology it created. Second, the fan-out shape: the signal relay fans out on a 2-participant topic (not a load stressor), while the actual fan-out-under-load surface is the tenant `/link` — one consumer receiving every call's targets, ~150 durable events/s at 50 calls/s. So the smaller cut (parties sub nothing; the link is the sole fan-out subscriber, and it *is* drained) is simultaneously lazier, leak-free, and a *better* measurement of the thing the roadmap names ("exercises link ... under load"). The permanent guard is the live run: `sends=32` proves 32 full lifecycles ran across 4 concurrent pairs (every start/accept/signal/hangup ok-acked, else `errors>0`), `set_targets=64` proves the link relay fired and the one consumer drained it, and `durable_closes=0` proves the link survived the churn. The run also surfaced a real Core behavior worth recording: a mid-call hangup that does *not* end the session re-emits `set_targets` (the observed 2 set_targets + 1 clear per call, `recvs=96 = 3×32`), so the link relay fires on every participant-set change, not just on accept — which only makes the `assert_calls` non-vacuity gate stronger.

### Verification (rule 4)

- `cargo fmt -p opn-loadgen --check` clean; `cargo clippy -p opn-loadgen --all-targets -- -D warnings` clean — `callchurn.rs` reuses `send`/`await_ack`, no new `unwrap`, no core-crate change; five linkdrop helpers + `ConnStats::errored` + `CLOSE_SLOW_CONSUMER` went `pub(crate)` (no behavior change).
- **24 loadgen unit tests green** (23 prior + 1 new: `set_targets_counts_only_set_targets_voice_frames`, covering set_targets vs clear vs non-voice vs garbage on the link matcher).
- **Live end-to-end run** (dev compose: Postgres 16 + Redis 7, one **debug** Core on the dev host as `opn_app`, throwaway 8-conn = 4-pair / 1 call-per-sec-per-pair / 8 s scenario): **`connections=5`** (4 pairs + 1 link consumer), **`sends=32`** (4 pairs × 8 rounds of a complete lifecycle), **`set_targets=64`, `recvs=96`** (the link drained every voice event), **`ack` n=32** (call-setup latency), `delivery_count=0` (parties sub nothing, by design), **`durable_closes=0`, `error_acks=0`, `errors=0`, `rate_limited=0`, `seq_gaps=0`, exit 0, PASS**. Non-vacuous on every axis: the FSM ran 32× concurrently, the link relay fired and was drained without a close, and every one of ~256 commands (8/round × 32 rounds) acked ok. Magnitudes (~20/28 ms setup) are debug-adjacent colocated noise and un-gated — this proves the *calls-driver + link path*, not a latency number.
- Drift gate untouched — `calls`/`calls_per_sec`/`assert_calls`/`set_targets` are loadgen-internal scenario/summary fields; the `calls.*` wire types were already on the wire (Sprint 6). No `.d.ts` moved.

### Exit criteria status (Sprint 10 — part B2 slice 3; this closes the **driver-mode** work of B2 — the remainder is operator/pinned-hardware)

| Criterion (roadmap Sprint 10) | Status |
|---|---|
| `call-churn` scenario exists and is runnable | **CLOSED** — `call-churn.json` + `just perf call-churn`; the calls driver + one-link-consumer mode built, unit-guarded (`set_targets` matcher), and live-proven (32 lifecycles, 64 set_targets, `durable_closes=0`). The 100-conn *run* is operator work. |
| `reconnect-storm` / `hot-channel` / `call-churn` scenarios (all three new driver modes) | **ALL CLOSED** — slices 1/2/3 of B2. Every Sprint 10 scenario now exists. |
| Profiling passes + fix/re-measure loop + tightened nightly smoke (item 7) | **OPEN (B2)** — operator/pinned-hardware work, evidence-first per item 4. |
| 24 h soak green | **OPEN (B2)** — machinery ready (B1 histogram + soak10x); the run is operator work. |

### Reflection

- **The seam held a seventh time — and this was the one it did NOT stretch to cover.** The prior six slices all grew or lightly forked the message driver. Calls is a different protocol, so the lazy-correct move was a *separate* driver early-returned from `run_scenario`, not a seventh `drive()` branch. Reuse still ran deep (linkdrop already spoke the frames), but the top-level cut was "don't force one loop to be two." The tell it was right: the 23 prior tests and every non-calls scenario are byte-identical.
- **The keeper was again a design realization from reading before writing — this time about the *test's* blast radius.** Every build slice's keeper (out-of-order delivery, replay-before-ack, member_add's missing cap, and now dead-sub accretion) came from reading the real semantics before writing the assertion/driver. This one is the subtlest: the naive faithful build would have measured a memory pathology it injected. The §15 budget keeps buying the reading.
- **All three B2 driver modes are done; what remains is not code.** Sprint 10's buildable machinery — the seq verifier (A), the soak-safe histogram (B1), and the three driver modes (B2 slices 1–3) — is complete. The profiling/fix loop, the tightened smoke, and the 24 h soak are runs on pinned hardware, the same operator-out-of-band shape as the fuzz burn-in. There is no more new loadgen *code* the release gate needs; there are runs.

### Not committed / next session

- **Sprint 10 part B2 slice 3 is complete and green but untracked** on top of committed 0–9 + Sprint 10 part A, and the untracked B1 + B2-slice-1 + B2-slice-2 stack. It adds `crates/loadgen/src/callchurn.rs` (the calls driver + link consumer + `set_targets` matcher + 1 unit test) and touches `driver.rs` (`set_targets` field, `errored`/`CLOSE_SLOW_CONSUMER` → `pub(crate)`), `main.rs` (the `calls`/`calls_per_sec`/`assert_calls` scenario fields, the `run_scenario` calls-branch early-return, the `Summary` `set_targets` wiring + `assert_calls` gate), `linkdrop.rs` (five helpers → `pub(crate)`), `scenarios/call-churn.json`, `perf/results/README.md`. No Cargo/justfile change. Drift gate untouched. The operator's call to land, as every slice.
- **Next: Sprint 10 part B2 is now all operator/pinned-hardware work** — no new loadgen code remains. In order: (1) the pinned-hardware profiling passes (`pg_stat_statements` → `cargo flamegraph` → `tokio-console`, evidence-first per item 4); (2) the fix + re-measure loop (each fix its own commit with before/after numbers); (3) tighten the nightly smoke to measured-p99 + 50 % (item 7) — and move `perf-smoke.yml`'s runtime Core to `opn_app` at the same time (the carry-over from part A); (4) the four heavy scenario runs on the pinned stack (`design`, `hot-channel`, `reconnect-storm`, `call-churn`) with results committed to `perf/results/`; (5) the 24 h 10× `soak10x`. After Sprint 10's gates are met, Sprint 11 (release engineering) is the last sprint.
- **Carried, non-blocking:** the seed loop still mints sessions *sequentially* (flagged since B1) — at 100/3000 conns the `call-churn`/`soak10x` seed will want a batched/concurrent mint before the heavy run; `perf-smoke.yml`'s runtime Core is still `opn_migrate` (RLS-bypassed) pending item 7.

---

## 2026-07-20 (part B2 slice 2) — Sprint 10 **part B2**, scenario slice 2 (`hot-channel`: the loadgen grows a group-fan-out topology — connection 0 `channels.create`s one group and `member_add`s the rest paced under the Social budget, all members subscribe to the single channel and send, so each send fans out to the whole group): built, verified live (36-conn dev-host run: `sends=144 → recvs=5184`, i.e. an exact **36× fan-out**, `seq_gaps=0`, `rate_limited=0`, `durable_closes=0`, exit 0); the 100-conn run + the last scenario (`call-churn`) + the profiling/fix loop deferred

Slice 1 (`reconnect-storm`) grew the drive loop into epochs — a new *connection lifecycle*, same paired-DM topology. Slice 2 is the first genuinely new *topology*: one channel with N subscribers instead of a two-member pair. It splits on the seam every scenario since Sprint 4 has taken, but this time the reuse ran the other way from slice 1: a group **member** is byte-identical to the existing `Pairing::Right` (wait for a channel id on a `oneshot`, then subscribe and send — a member does exactly that), so the *only* new driver code is a `GroupCreator` that creates the channel, adds everyone, and broadcasts the id to all of them. No `hot-channel.rs` mode, no new member variant. This is roadmap item 1's `hot-channel` bullet — "one 100-member group at 10 msg/s (fan-out 1000 evt/s) — the fan-out stress shape" — verbatim.

### What exists now

- **A `GroupCreator` pairing + one match arm** (`crates/loadgen/src/driver.rs`) — the `Pairing` enum gained `GroupCreator { member_ids, txs }`; the channel-establish `match` in `drive()` gained one arm that `channels.create`s the group with the first ≤32 members (Core auto-adds the creator, §10.2), `member_add`s the remainder, then sends the resulting `channel_id` to every member's `oneshot`. **Group members reuse `Pairing::Right` unchanged** — a member's whole job (await the id → sub → send at rate) is exactly what `Right` already does, so the epoch/send/read machinery, `SeqTracker`, `drain_sub`, and the histograms are all inherited verbatim. The only genuinely new code is the creator's setup path.
- **Paced `member_add` under the Social budget** (`driver.rs`, `MEMBER_ADD_PERIOD = 200 ms`) — `member_add` is the Social rate class (5/s sustained, burst 20, §12). The creator sleeps 200 ms between adds (exactly the sustained rate), so even a 100-member group's ~67 adds build without ever tripping a `rate_limited` ack — the burst-20 headroom absorbs scheduling jitter. Every add's ack is asserted `ok`, so a silently-failed add surfaces as a connection error, not a phantom-missing member.
- **`split_group_members(ids, create_max)`** (`driver.rs`, pure) — splits the member list into the `create` list (≤`GROUP_CREATE_MAX = 32`, a mirror of Core's `MEMBERS_MAX`) and the `member_add` remainder. Pure and unit-tested at the boundary (empty / under / exactly-at / over the cap) — the one piece of the setup that can be got wrong off by one.
- **`main.rs` group wiring** — a `group: bool` scenario field (serde default `false`); when set, `run_scenario` builds one group of every connection (conn 0 = `GroupCreator` with the other N-1 char ids + their `oneshot` senders; conns 1..N = `Right` receivers) instead of pairs. The `base` `ConnConfig` factory was hoisted above the topology branch so both paths share it. Non-group scenarios are byte-identical (the even-count rounding, the pair loop, every field). Two guards in `validate`: `group` needs ≥2 connections (creator + ≥1 member) and is mutually exclusive with `reconnect_at_secs` (no group-storm scenario exists).
- **`crates/loadgen/scenarios/hot-channel.json`** (junior) — the real 100-conn spec: `group: true`, 10 msg/s aggregate, `warmup_secs: 25` (covers the create + ~67 paced adds before the aligned first send), gates `assert_no_seq_gaps` + `assert_no_durable_closes`, `assert_ack_p99_ms` omitted (item 7 tightens it after the fix loop — same call as `soak10x.json`). Runs via `just perf hot-channel` for free (the scenario-parameterized recipe from B1). Committed at 100 conns, verified at 36 (soak10x/reconnect-storm precedent: commit the spec, the heavy run is the operator's).
- **`perf/results/README.md`** (junior) — a `hot-channel` scenario bullet after the `reconnect-storm` one.

### Decisions closed during implementation (roadmap deviations, all ponytail)

1. **A group *member* is the existing `Right`; only the *creator* is new.** The reflexive move is a symmetric `GroupCreator`/`GroupMember` pair. But a member does precisely what `Right` already does — block on a `oneshot` for a channel id, then subscribe and send — so a second variant would be a copy. Reusing `Right` means the entire send/read/epoch/`SeqTracker`/histogram body is inherited with zero change, and the driver diff collapses to one enum variant + one match arm. The proof it's the right cut: the 22 prior driver tests and every non-group scenario are untouched.
2. **Build the 100-member group with `create(32)` + paced `member_add`, not a 33-member cap.** The roadmap wants a 100-member fan-out; `channels.create` caps its member list at 32 (§10.2, Sprint 4). Reading *both* the create handler and the `member_add` store fn closed the apparent contradiction: `member_add` has **no total cap**, so a group grows past 33 only through it. Capping the loadgen at 33 would have under-delivered the fan-out shape by 3×; this is the keeper (below).
3. **200 ms pace on `member_add`, not fire-and-forget.** `member_add` is Social-classed (5/s). A burst of 67 adds would exhaust the burst-20 bucket and start collecting `rate_limited` acks mid-setup — a flaky, rate-dependent setup. Pacing at exactly the sustained rate (200 ms) makes setup deterministic and rate-limit-free; the live run confirmed `rate_limited=0`. Reading the rate-class map *before* writing the loop is what turned a latent flake into a non-issue.
4. **`group` is one flag, one group — not a `group_size`/`num_groups` matrix.** The roadmap names "one 100-member group". A single boolean (all `connections` join one channel) is the whole requirement; multiple groups or a configurable size is YAGNI. `connections` already *is* the group size.
5. **Debug Core for the verify, like B1.** This slice proves the group/fan-out/continuity *path*, not a latency number, so a debug Core (far faster to compile than release, `just` being absent on the dev host) is the right tool — the same call B1 made. The magnitudes (~59/107 ms) are meaningless and un-gated; the shape (36× fan-out, zero gaps) is the evidence.
6. **Delegated the two disjoint mechanical files to a parallel junior; kept the driver surgery.** `hot-channel.json` and the README bullet touch no `.rs` and were pinned to the exact new field name (`group`) so the junior couldn't guess it wrong — run in parallel while I did the `GroupCreator` arm, the `member_add` pacing, and the `split_group_members` boundary. Reviewed both returned files against disk (parsed the JSON, confirmed `group: true` and the omitted ack gate) before trusting them; clean this time.

### The keeper this session: the roadmap's "100-member group" vs the design's "create caps at 32" — resolved only by reading `member_add`'s store fn, not by capping the loadgen

The obvious way to build a group is one `channels.create`. Read only that handler and you conclude the design caps a group at 33 members (creator + 32), and you either write a 33-member `hot-channel` (a 330 evt/s fan-out, a third of the roadmap's 1000) or you go amend the design. Both are wrong. Reading the *second* function — `store::member_change` — shows `member_add` inserts with **no total-member ceiling**, so a 100-member group is fully reachable and design-legal; the 32 is a *create-list* cap, not a *group-size* cap. That one fact is the whole shape of the slice: it means the creator's job is `create(≤32)` **then** `member_add` the rest, which in turn means the adds hit the Social rate bucket, which is why they must be paced. Every downstream decision (the split helper, the 200 ms pace, the 25 s warmup) falls out of reading two functions instead of one. Getting it wrong doesn't fail a test — it silently ships a scenario that stresses the fan-out at a third of its target, which looks fine and measures the wrong thing. The permanent guard is the live run itself: `sends=144 → recvs=5184` is an exact 36× multiplier, proving one send reached all 36 subscribers, and `seq_gaps=0` proves that fan-out lost nothing under concurrent senders. The `split_group_members` unit test guards the off-by-one at the cap boundary that the pacing path depends on.

### Verification (rule 4)

- `cargo fmt -p opn-loadgen --check` clean; `cargo clippy -p opn-loadgen --all-targets -- -D warnings` clean — the `GroupCreator` arm reuses `send`/`await_ack`, no new `unwrap`, no core-crate change; `split_group_members` is a one-line `split_at`.
- **23 loadgen unit tests green** (22 prior + 1 new: `group_members_split_at_the_create_cap`, covering empty / under-cap / exactly-cap / over-cap).
- **Live end-to-end run** (dev compose: Postgres 16 + Redis 7, one **debug** Core on the dev host as `opn_app`, throwaway 36-conn / 10 msg/s / 10 s group scenario — creator + 35 members, so `create(32)` + 3 paced `member_add`s): **`connections=36`, `sends=144`, `recvs=5184` (= 36× fan-out), `delivery_count=5040` (= 144 × 35 peer deliveries, self-loopback correctly excluded), `seq_gaps=0`, `rate_limited=0`, `durable_closes=0`, `error_acks=0`, `errors=0`, exit 0, PASS**. Non-vacuous on every axis: the group formed and grew past the create cap (the 3 `member_add`s all acked, else `errors>0`), each send fanned out to the whole membership (36× recvs), the paced adds never tripped a rate limit (`rate_limited=0`), and continuity held under the 36-way fan-out with concurrent senders (`seq_gaps=0`). Magnitudes (~59/107 ms) are debug-adjacent colocated noise and deliberately un-gated.
- Drift gate untouched — `group` is a loadgen-internal scenario field; the `channels.create`/`member_add`/`channels.message` wire types were already on the wire (Sprint 3/4). No `.d.ts` moved.

### Exit criteria status (Sprint 10 — part B2 slice 2; the rest of B2 = 100-conn run + `call-churn` + profiling/fix loop + soak)

| Criterion (roadmap Sprint 10) | Status |
|---|---|
| `hot-channel` scenario exists and is runnable | **CLOSED** — `hot-channel.json` + `just perf hot-channel`; the group-fan-out driver mode built, unit-guarded (`split_group_members`), and live-proven (36× fan-out, `seq_gaps=0`). The 100-conn *run* is operator work. |
| `call-churn` scenario | **OPEN** — the last driver-mode slice (a calls driver: `start`/`accept`/`signal`/`hangup` at ~1 Hz). |
| Profiling passes + fix/re-measure loop + tightened nightly smoke | **OPEN (B2)** — operator/pinned-hardware work, evidence-first per item 4. |
| 24 h soak green | **OPEN (B2)** — machinery ready; the run is operator work. |

### Reflection

- **The seam held a sixth time, and reuse ran the opposite way from slice 1.** `reconnect-storm` grew the existing loop (a new lifecycle, same topology); `hot-channel` needed a new topology but discovered the *member* half was already built (`Right`). Both are the ponytail ladder picking the higher rung — the tell each time is that every untouched scenario still runs byte-identical.
- **The value was reading two functions, not one.** The whole slice hinges on `member_add` having no total cap — a fact invisible from the `create` handler alone. Reading one function would have shipped a plausible-looking scenario measuring a third of the intended load. The §15 budget buys the second read.
- **Delegation held.** Mechanical files to a parallel junior with the coupling field pinned, driver surgery and every rate/cap call to me, review-before-trust against disk — clean again.

### Not committed / next session

- **Sprint 10 part B2 slice 2 is complete and green but untracked** on top of committed 0–9 + Sprint 10 part A, and the untracked B1 + B2-slice-1 stack. It touches `crates/loadgen/src/driver.rs` (the `GroupCreator` variant + match arm, `split_group_members`, the `MEMBER_ADD_PERIOD`/`GROUP_CREATE_MAX` consts, +1 unit test) + `main.rs` (the `group` field, the hoisted `base`, the group launch branch, the two validate guards) + `scenarios/hot-channel.json` + `perf/results/README.md`. No Cargo/justfile change (reused B1's scenario-parameterized `just perf`). Drift gate untouched. The operator's call to land, as every slice.
- **Next: Sprint 10 part B2, slice 3 — `call-churn`.** The last new driver mode: a calls driver (`calls.start` → notify ring → `calls.accept` → `calls.signal` both ways → `calls.hangup`) at ~1 Hz, exercising the call FSM + the `/link` relay under load. It is the one remaining scenario that needs genuinely new protocol driving (the `--link-drop` checker already speaks most of these frames — a reuse candidate to scout first). Then the operator's pinned-hardware work: profiling (`pg_stat_statements` → `cargo flamegraph` → `tokio-console`, evidence-first), the fix+re-measure loop, tightening the nightly smoke to measured-p99 + 50 % (item 7, and move `perf-smoke.yml`'s runtime Core to `opn_app` at the same time — the carry-over from part A), and the 24 h 10× soak.
- **Carried, non-blocking:** the seed loop still mints sessions *sequentially* (flagged in B1) — at 100/3000 conns the `hot-channel`/`soak10x` seed will want a batched/concurrent mint before the heavy run; `perf-smoke.yml`'s runtime Core is still `opn_migrate` (RLS-bypassed) pending item 7.

---

## 2026-07-20 (part B2 slice 1) — Sprint 10 **part B2**, scenario slice 1 (`reconnect-storm`: the loadgen driver grows a drop→reconnect→resume mode — a multi-epoch connection loop that drops its socket at a storm instant, reconnects after a deterministic 0–3 s stagger, and resumes with `sub last_seq`, with seq continuity checked *through* the reconnect): built, verified live (20-conn dev-host run, `reconnects=20`, `resume_count=20`, `seq_gaps=0`, exit 0); the 3000-conn run + the two other B2 scenarios (`hot-channel`, `call-churn`) + the profiling/fix loop deferred

Part B1 closed the last piece of *measurement* machinery (the soak-safe histogram). B2 is the four remaining scenarios and the pinned-hardware runs, and it splits on the **same new-driver-mode seam** every scenario since Sprint 4 has taken: each of B2's three remaining scenarios (`reconnect-storm`, `hot-channel`, `call-churn`) needs a genuinely new *protocol driver mode*, and the profiling/fix loop + the 3000-conn/24 h runs are operator work on pinned hardware. This slice is the first driver mode — and the highest-reuse of the three, which is exactly why it goes first: it keeps the paired-DM topology (no new channel setup), reuses `SeqTracker` verbatim for continuity, and reuses the resume protocol (`sub last_seq`, already exercised by the kill9 `--verify-resume` verifier). The only genuinely new code is the *reconnect capability in the drive loop*. `hot-channel` (a 100-member group → a new setup topology) and `call-churn` (a calls driver) are the next two slices, each on the same seam. No roadmap amendment — this is roadmap item 1's `reconnect-storm` bullet + item 6's gate 5 ("all resumed within 60 s, no replay gaps; loadgen verifies seq continuity per client"), verbatim.

### What exists now

- **A multi-epoch drive loop** (`crates/loadgen/src/driver.rs`) — `drive()` was a single connect→auth→establish-channel→sub→loop. It is now an **epoch loop**: `connect_and_auth` (extracted, shared by initial setup and every reconnect) → establish the pair channel *once* → then a loop of `[ (re)subscribe → run the send/read select loop until it breaks Done or Reconnect ]`. A non-storm scenario (`reconnect_at = None`) runs **exactly one epoch**, byte-identical to the old behavior (the storm select-arm is disabled by an `if` guard, and the inner loop is now a `loop { … break EpochOutcome::X }` expression rather than a `mut outcome` variable — no `unused_assignments`). On a storm the inner loop breaks `Reconnect`; `drive()` hard-drops the split socket halves (the realistic resource/network failure — no graceful close), sleeps the per-connection stagger, clears `pending` (in-flight RTTs on the dead socket are unrecoverable), reconnects via `connect_and_auth`, and loops back to a resume `sub`.
- **`drain_sub`** (`driver.rs`, new) — the load-bearing correctness piece (see the keeper). The resume `sub`'s replayed `channels.message` pushes arrive *before* the sub ack (§4.4); the existing `await_ack` **skips pushes**, so reusing it would silently discard the entire replay and manufacture false seq holes. `drain_sub` instead feeds every pre-ack frame to `handle_incoming` (folding replayed seqs into the trackers) and returns on the sub ack. It records those replays with **`record_delivery = false`** — a replayed message's embedded timestamp is pre-storm, so its "latency" is the whole reconnect gap and would poison the live-delivery histogram. `handle_incoming` gained that one `record_delivery` bool; the hot loop passes `true`.
- **`SeqTracker::frontier()`** (`driver.rs`, new) — the resume watermark. Resuming from `frontier` (the highest fully-contiguous seq, not `max_seen`) re-requests any pre-storm hole above it rather than skipping it. The tracker map **persists across the reconnect** — that is precisely what turns `seq_gaps` into a per-client continuity check *through* the storm.
- **`ConnStats::resume` (histogram) + `reconnects` (counter)** (`driver.rs`) — resume round-trip (re-sub → sub ack, i.e. gap-drain time) into a fixed-memory `hdrhistogram` (empty in every non-storm scenario), and a per-connection reconnect count. `ConnConfig` gained `reconnect_at: Option<Instant>` + `reconnect_delay: Duration`.
- **`main.rs` wiring** — scenario fields `reconnect_at_secs: Option<u64>` / `reconnect_jitter_secs` (default 3.0) / `assert_reconnected`; a validation that `reconnect_at_secs < duration_secs` (there must be a post-storm window to prove resume); a **deterministic** jitter spread (`reconnect_delay(i) = jitter · i/conns` — the thundering herd without an rng, matching the driver's rng-free send mix); `resume_*` percentiles + `reconnects` in both the JSON summary and the human table; and the `assert_reconnected` non-vacuity gate (fail if the run reported 0 reconnects — the storm never fired).
- **`crates/loadgen/scenarios/reconnect-storm.json`** (new, junior) — the real 3000-conn/300-msg-s spec (matching the roadmap's "drop all 3000"), 90 s with the storm at 40 s, `warmup_secs: 30`, `assert_reconnected` + `assert_no_seq_gaps` + `assert_no_durable_closes`. The "resumed within 60 s" target is **measured** (`resume_*`), not hard-gated — item 7 tightens perf thresholds after the fix loop, same call as `design.json`/`soak10x.json`. Runs via `just perf reconnect-storm` for free (the scenario-parameterized recipe from B1; no justfile change).
- **`perf/results/README.md`** (junior) — a `reconnect-storm` scenario bullet + gate 5 expanded with the machinery note.

### Decisions closed during implementation (roadmap deviations, all ponytail)

1. **Reuse the paired-DM driver as a multi-epoch loop — do not fork a `reconnect.rs`.** The reconnect is a *capability the existing drive loop grows*, not a new tool: the storm keeps the same channel, the same trackers, the same send mix. A separate driver mode (the shape `--verify-resume`/`--xinstance`/`--link-drop` took) is right when the fault or topology is genuinely new; here only the connection lifecycle changes, so an epoch loop over the *same* body is the smaller diff. The proof it's the right cut: `reconnect_at = None` runs one epoch, byte-identical — every other scenario is unaffected, and the 22-test suite plus the design/smoke/soak drivers need no change.
2. **`drain_sub`, not `await_ack`, for the resume sub — the keeper.** Detailed below. In one line: `await_ack` discards pre-ack pushes, and the resume replay *is* pre-ack pushes (§4.4), so reusing it would drop the gap and fail the continuity gate on correct behavior.
3. **Replayed messages don't record delivery latency (`record_delivery = false`).** Their timestamps are pre-storm; counting them would inflate the delivery histogram by the whole reconnect gap. Visible in the live numbers: `delivery_max = 21 ms`, not the ~seconds a recorded replay would show. This is the difference between the delivery p99 staying a *live-delivery* number and quietly becoming a *catch-up* number.
4. **Resume from `frontier`, not `max_seen`.** `frontier` re-requests any seq stuck above a pre-storm hole; `max_seen` would skip it. One new accessor, one unit test (`frontier_is_the_resume_watermark`, including the `[1,3] → Some(1)` hole case).
5. **Hard socket drop, cached-token re-auth — no graceful close, no re-mint.** Dropping the split halves closes the TCP socket: the realistic resource/network blip the storm models. Re-auth uses the same still-valid (10-min JWT) token — a client reconnecting *within* token life doesn't re-mint. The roadmap's "token-mint burst" is the *expired-token* worst case and stresses the HTTP mint endpoint, an orthogonal subsystem from the WS resume path this gate covers (item 6 gate 5 is continuity + all-resumed, not mint throughput). A `remint_on_reconnect` toggle is a cheap follow-on if the operator wants to stress mint under burst — noted, not built (YAGNI; the gated property is fully covered by re-auth + resume).
6. **Soft resume gate (measured) + a hard `assert_reconnected` non-vacuity gate.** The "within 60 s" resume target is measured (`resume_*` in the summary) but not hard-asserted until item 7 measures a real threshold on pinned hardware — the same "don't assert a number you haven't measured" call `design.json` made for ack-p99. The hard gates are `assert_no_seq_gaps` (the real correctness invariant) + `assert_no_durable_closes`, plus `assert_reconnected` as the vacuity guard (mirrors pg-restart's `assert_error_acks`): a reconnect-storm that reports 0 reconnects tested nothing.
7. **Committed at the real 3000-conn spec, verified at 20 (soak10x precedent).** The committed scenario is the faithful "drop all 3000"; the heavy run joins the operator's part-B2 pinned-hardware battery. The machinery is fully verifiable at 20 conns on the dev host, exactly as B1/part-A verified with 20-conn throwaways.
8. **Delegated the two disjoint mechanical files to a parallel junior; kept the driver surgery.** `reconnect-storm.json` and the README bullet are reference-driven and touch no `.rs` — a clean junior task, run in parallel while I did the epoch-loop refactor, the `drain_sub`/`record_delivery` correctness reasoning, and the `frontier` semantics. Reviewed both returned files before trusting them; clean this time — no correction needed (the field names were pinned in the spec precisely so the junior couldn't guess them wrong).

### The keeper this session: a false-**FAIL** hiding in a natural reuse — `await_ack` on the resume sub would discard the replay and trip the continuity gate on correct behavior

Like part A (a *build* slice), this slice's keeper is a **design** realization caught by reading the protocol before writing the code, not a red test after. The obvious way to write the resume sub is to reuse `await_ack` — it already exists, and the first-epoch sub uses it. But `await_ack` loops reading and returns only on the matching ack, **discarding every push it passes** (`Some(Ok(_)) => {}`). The resume replay is delivered as `channels.message` pushes *before* the sub ack (§4.4, the same rule the kill9 `--verify-resume` verifier drains by hand). So an `await_ack`-based resume would silently swallow the entire gap replay; then the first live message after reconnect arrives with a seq above the now-stale frontier, `SeqTracker` sees an open hole, and `seq_gaps > 0` — the gate **fails on a perfectly healthy resume**. That is the same class as part A's out-of-order keeper: a release gate that trips on correct behavior, the most expensive kind of wrong because it looks like rigor. The catch came from tracing the resume protocol (replay-before-ack) *before* writing the sub, which forced `drain_sub` to exist. The permanent guards are `frontier_is_the_resume_watermark` (the watermark logic in isolation) and the **live run itself**: 20 reconnects, `resume_count = 20`, `seq_gaps = 0` — the replay was drained and continuity held across every storm. A second, smaller keeper rode along: `record_delivery = false` for replays, without which the delivery p99 would have quietly absorbed the reconnect gap (the live `delivery_max = 21 ms`, not seconds, is the evidence it works).

### Verification (rule 4)

- `cargo fmt -p opn-loadgen --check` clean (after one auto-format); `cargo clippy -p opn-loadgen --all-targets -- -D warnings` clean — the inner loop is a `loop`-expression (no `unused_assignments`), `drain_sub`/`handle_incoming` carry the one `#[allow(clippy::too_many_arguments)]` the module already used, no core-crate change.
- **22 loadgen unit tests green** (21 prior + 1 new: `frontier_is_the_resume_watermark`, covering empty→`None`, contiguous, non-1 first-seen, the `[1,3]→Some(1)` pre-storm hole, and out-of-order-that-resolves). The two `handle_incoming` test call sites were updated for the new `record_delivery` arg.
- **Live end-to-end run** (dev compose: Postgres 16 + Redis 7 + MinIO, one **release** Core on the dev host as `opn_app`, 20 conns / 20 msg/s / 12 s, storm at 5 s with 1 s jitter): **`reconnects=20`, `resume_count=20` (resume RTT p50 1.02 / p99 42.27 / max 42.27 ms), `seq_gaps=0`, sends 250 / recvs 500, 0 durable closes, 0 errors, exit 0, PASS**. Non-vacuous on every axis: the storm fired for all 20 (`reconnects=20`), each drained a replay (`resume_count=20`), each pair's 250 sends were received by both members across the reconnect (500 recvs), and continuity held through every reconnect (`seq_gaps=0`). Magnitudes are meaningless (debug-adjacent colocated generator) and deliberately un-gated — this proves the *reconnect/resume/continuity path*, not a latency number.
- Drift gate untouched — `resume_*`, `reconnects`, and the reconnect scenario fields are all loadgen-internal; no contracts wire type changed, no `.d.ts` moved.

### Exit criteria status (Sprint 10 — part B2 slice 1; the rest of B2 = 3000-conn run + `hot-channel` + `call-churn` + profiling/fix loop + soak)

| Criterion (roadmap Sprint 10) | Status |
|---|---|
| `reconnect-storm` scenario exists and is runnable | **CLOSED** — `reconnect-storm.json` + `just perf reconnect-storm`; the drop→reconnect→resume driver mode built + unit-guarded (`frontier_*`) + live-proven (`reconnects=20`, `seq_gaps=0`). The 3000-conn *run* is operator work. |
| Reconnect-storm: all resumed, no replay gaps (item 6 gate 5) | **Machinery CLOSED** — per-client seq continuity through the reconnect is now a hard gate (`assert_no_seq_gaps` over `SeqTracker` state that survives the storm); resume latency is measured (`resume_*`). The "< 60 s" *threshold* is measured-not-gated until item 7. |
| `hot-channel` / `call-churn` scenarios | **OPEN** — each is the next driver-mode slice (group topology; calls driver). |
| Profiling passes + fix/re-measure loop + tightened nightly smoke | **OPEN (B2)** — operator/pinned-hardware work, evidence-first per item 4. |
| 24 h soak green | **OPEN (B2)** — machinery ready (B1 histogram + soak10x); the run is operator work. |

### Reflection

- **The seam held again — and this time it favored reuse, not a new tool.** The prior four chaos slices each added a *new* driver mode because their fault/topology was new. `reconnect-storm`'s "fault" is just the connection lifecycle, so the lazy-and-correct move was to grow the existing drive loop into epochs, not fork a fifth mode. The tell that it was the right cut: every non-storm scenario runs one epoch, byte-identical, and needed zero changes. Picking the higher rung (reuse) over the reflexive one (another `*.rs` mode) is the ponytail ladder working.
- **A build slice's keeper is a false-gate avoided, twice running.** Part A's was out-of-order-delivery; this one's is replay-before-ack. Both are release gates that would trip on *correct* behavior, both caught by reading the real protocol before writing the assertion, both invisible to a green unit suite until the live run. The §15 budget keeps buying the reading, and the reading keeps being where the value is.
- **Delegation held.** Mechanical files to a parallel junior, the driver surgery and every correctness call to me, review before trust — clean this time because the spec pinned the coupling (the new field names) the junior would otherwise have guessed.

### Not committed / next session

- **Sprint 10 part B2 slice 1 is complete and green but untracked** on top of committed 0–9 + Sprint 10 part A + part B1. It touches `crates/loadgen/src/driver.rs` (epoch loop, `connect_and_auth`, `drain_sub`, `frontier`, `resume`/`reconnects`, `record_delivery`) + `main.rs` (scenario fields, jitter spread, `resume_*`/`reconnects` summary, `assert_reconnected`) + `scenarios/reconnect-storm.json` + `perf/results/README.md`. No Cargo/justfile change (reused B1's `hdrhistogram` + scenario-parameterized `just perf`). Drift gate untouched. The operator's call to land, as every slice.
- **Next: Sprint 10 part B2, slice 2 — `hot-channel`.** It needs the first genuinely new *topology*: `channels.create` + one shared ~100-member group, driven at ~10 msg/s for the 1000 evt/s fan-out shape (the current driver only knows paired `open_direct` DMs). Then slice 3 — `call-churn` (a calls driver: `start`/`accept`/`signal`/`hangup` at ~1 Hz). Then the operator's pinned-hardware work: profiling (`pg_stat_statements` → `cargo flamegraph` → `tokio-console`, evidence-first), the fix+re-measure loop (each fix its own commit with before/after numbers), tightening the nightly smoke to measured-p99 + 50 % (item 7, and move `perf-smoke.yml`'s runtime Core to `opn_app` at the same time — the carry-over from part A), and the 24 h 10× soak.
- **Carried, non-blocking:** the seed loop still mints sessions *sequentially* — at 3000 conns the `reconnect-storm`/`soak10x` seed will want a batched/concurrent mint before the real run (flagged in B1); the `remint_on_reconnect` mint-burst variant is a cheap toggle if the operator wants to stress the HTTP mint endpoint under the storm; `perf-smoke.yml`'s runtime Core is still `opn_migrate` (RLS-bypassed) pending item 7.

---

## 2026-07-20 (part B1) — Sprint 10 **part B1** (loadgen soak-safety: the flagged unbounded-`Vec` latency store replaced with fixed-memory `hdrhistogram` + the `soak10x` scenario + a scenario-parameterized `just perf`): built, verified live (real p50/p99/max out of the histogram, `seq_gaps=0`, exit 0); the 3000-conn/24 h soak *run* + the three other scenarios + the profiling/fix loop (part B2) deferred

Part A built the measurement machinery (the seq-continuity verifier, the `design` scenario, the core-pinned stack, the results discipline). Part B splits again on the **same machinery-vs-run seam**: **(B1)** the *last piece of buildable, locally-verifiable machinery the heavy runs need* — the soak-safe latency store, without which no long run can even complete; **(B2)** the actual profiling passes, the fix+re-measure loop, the tightened nightly gate, the three remaining scenarios (each needing a genuinely new *protocol driver* mode), and the 24 h run on pinned hardware (operator work, the same shape as the 24 h fuzz burn-in and the soak). B1 is the honest smallest cut: the v0 percentile store is an unbounded `Vec<u64>` per connection — flagged latent-OOM in `main.rs`'s header since Sprint 4 ("the Vec is fine to ~1M samples; the 24 h soak wants hdrhistogram before it records hundreds of millions"). `soak10x` records tens of millions of samples and would OOM around hour six — the one run you can least afford to lose. That is a prerequisite *bug*, self-contained, fully unit + live verifiable on the dev host. No roadmap amendment — B1 is item 1's `soak10x` scenario + the test plan's implicit "the histogram must survive the soak it measures" (Sprint 4 item 9 named `hdrhistogram` for exactly this).

### What exists now

- **Bounded latency store** (`crates/loadgen/src/driver.rs`, `main.rs`) — `ConnStats`'s `ack_rtts_us: Vec<u64>` / `deliveries_us: Vec<u64>` are gone, replaced by `ack` / `delivery` as `hdrhistogram::Histogram<u64>` (range 1 µs .. 60 s, 3 significant figures). Fixed memory per connection *regardless of sample count* — the whole point. A single `new_hist()` is the only constructor; `Default` is hand-written because `Histogram` has none. Recording is `saturating_record` (decision 3). `Summary::merge` accumulates per-connection histograms losslessly with `Histogram::add` (same fixed bounds ⇒ `add` cannot error), and `Percentiles::of(&Histogram)` reads `value_at_quantile(0.50/0.99)` + `max()`. The old sort-based `pct_ms` helper is deleted.
- **`crates/loadgen/scenarios/soak10x.json`** (new, junior) — Sprint 10 item 1's `soak10x`: 3000 conns, 300 msg/s aggregate, 86 400 s (24 h), `warmup_secs: 30` (3000 conns need a wide setup barrier), `assert_no_seq_gaps` + `assert_no_durable_closes`. `assert_ack_p99_ms` **omitted entirely** — item 7 tightens the ack gate to measured-p99 + 50 % only *after* the fix loop; asserting an unmeasured threshold is wrong (same call as `design` in part A). Its `_comment` records that item 6's RSS-slope / fd / p99-drift / zero-janitor-failure targets are operator-**observed**, not loadgen assertions.
- **`just perf` is scenario-parameterized** (justfile) — `perf scenario="design"`, so `just perf` is unchanged and `just perf soak10x` runs the soak; the summary lands in `perf/results/<date>-<scenario>.json`. The pre-auth caps are raised to 4000 (`OPN_PREAUTH_PER_IP_MAX` **and** the previously-unset `OPN_PREAUTH_GLOBAL_MAX`, whose default 1000 would reject 3000 conns) — the trusted single-source generator must never be admission-throttled (decision 4).
- **`perf/results/README.md`** (junior) — a `## Scenarios` section listing `design` + `soak10x`, and the histogram note (bounded fixed-memory buckets vs the unbounded sorted `Vec`, quoting the latent-OOM the swap closes).

### Decisions closed during implementation (roadmap deviations, all ponytail)

1. **`hdrhistogram`, not a reservoir sampler — the load-bearing dep call.** The purist "no new dep" move is reservoir sampling (`rand` is already transitive). But a *correct* reservoir sampler is *more* code (RNG seeding + algorithm R + percentile-over-sample) **and** *less* accurate (sampling error on p99), while `hdrhistogram` is deterministic, exact-to-3-sigfig, mergeable, and the tool the roadmap **named in Sprint 4 item 9**. ponytail's own rule — "two options the same size, take the one correct on edge cases; lazy is writing less code, not picking the flimsier algorithm" — points at `hdrhistogram` on all three axes (less code, more correct, roadmap's choice). The only cost is a dependency edge, and `default-features = false` drops the serialization/sync features (and their flate2/nom/base64/crossbeam deps), leaving just `num-traits` + `byteorder` — both already on the cargo-deny allow-list, so the Sprint 9 supply-chain gate stays green.
2. **Not a hand-rolled log-linear bucket histogram.** That is literally reinventing `hdrhistogram` — the exact anti-ponytail (reimplementing a crate that already lives one `cargo add` away). Rejected on sight.
3. **`saturating_record`, not `record`.** A latency above 60 s is a hang, and it should surface as `max`, not vanish. `record()` returns `Err` and *drops* an out-of-range sample; `saturating_record` clamps it to the top bucket and keeps it. For a soak whose whole job is catching tail pathologies, silently dropping the worst samples is the wrong default.
4. **`just perf` takes a scenario arg; it does not gain a duplicated `soak` recipe.** A second recipe would copy the whole 70-line orchestration body — two owners of one job, the thing Sprint 11 item 2 explicitly warns against. A default-valued `scenario` parameter keeps `just perf` byte-identical for callers and adds `just perf soak10x` for free. The cap bump rides along because 3000 conns from one IP trip *both* pre-auth caps, and the admission gate is not what a perf run measures.
5. **Delegated the two independent files to a parallel junior; kept the histogram surgery.** `soak10x.json` and the perf README are mechanical, reference-driven, and touch no `.rs` — a clean junior task, run in parallel on disjoint files while I did the `ConnStats`/`Summary` rework (the manual `Default`, the merge-cannot-error reasoning, the `unwrap_used`-deny interaction that forced `expect` in `new_hist`, and the exact-→-bucketed percentile-semantics shift). Reviewed both returned files before trusting them; this time they were clean — no correction needed.

### The keeper: a latent OOM that only bites at hour six of the run you can least afford to lose

Unlike a code fix caught by a red test, this slice's keeper is the *class* of bug the change closes and the semantics shift it had to get right. The unbounded `Vec` grows one `u64` per acked send **and** one per received delivery: at the soak's rate that is ~50 M entries over 24 h, ~400 MB of sample vectors — an OOM that manifests not on the dev host's 12 s smoke but ~6 hours into the exact 24 h release-gate run whose failure costs a day. Fixing it is a swap to fixed-memory buckets. The subtle part is that the swap changes the percentile *semantics*: v0 was **exact** nearest-rank over a sorted `Vec`; `hdrhistogram` is **bucketed** to 3 significant figures. The permanent guard is `percentiles_from_histogram` — 1..=100 µs recorded, asserting p50 ≈ 50 µs / p99 ≈ 99 µs / max = 100 µs — which holds because values under 1000 with 3 sig figs each occupy their own bucket (still exact where it is tested), and the perf gates live far above 3-sigfig resolution. The live run confirmed the merged multi-connection summary emits sane percentiles, with `delivery_p99` landing *exactly* on 56.0 ms — a visible bucket boundary, the expected bucketing signature, not a bug. Getting the bucket read backwards (low edge vs high edge) would have quietly drifted every future perf number; the unit test is the thing that would catch that regression.

### Verification (rule 4)

- `cargo fmt -p opn-loadgen --check` clean (after one auto-format of the two `saturating_record` call sites); `cargo clippy -p opn-loadgen --all-targets -- -D warnings` clean — `new_hist` uses `expect` (allowed; `unwrap_used` bans only `.unwrap()`), the two `Histogram::add` merges are `let _ =` (the `#[must_use]` Result is provably-infallible here), no core-crate change.
- **21 loadgen unit tests green** (count unchanged: `percentiles_nearest_rank` became `percentiles_from_histogram`, `percentiles_empty_is_zero` and the two driver-classifier tests were adapted to the `Histogram` API; the 5 `SeqTracker` tests and the ack-classifier tests are untouched).
- **Live end-to-end run** (dev compose: Postgres 16 + Redis 7 + MinIO, one **debug** Core on the dev host, 20 conns / 20 msg/s / 12 s): **260 sends / 520 recvs, ack n=260 p50 16.48 / p99 57.82 / max 57.92 ms, delivery n=260 p50 16.53 / p99 56.0 / max 56.77 ms, `seq_gaps` 0, 0 durable closes, exit 0, PASS**. Non-vacuous: 520 deliveries flowed through both histograms and merged across 20 connections into real percentiles. The ~16/57 ms magnitudes are meaningless (debug build, colocated generator) and deliberately un-gated — this run proves the *histogram summary path*, not a latency number, exactly as part A's throwaway did.
- **cargo-deny surface**: only `hdrhistogram` (MIT/Apache-2.0), `byteorder` (MIT/Unlicense), `num-traits` (MIT/Apache-2.0) added — all already permitted by `deny.toml`'s allow-list.
- Drift gate untouched — the histogram and `soak10x` are loadgen-internal; no contracts wire type changed, no `.d.ts` moved.

### Exit criteria status (Sprint 10 — part B1 slice; part B2 = the runs + the three driver scenarios + the fix loop)

| Criterion (roadmap Sprint 10) | Status |
|---|---|
| Soak-safe measurement (prerequisite for the 24 h soak) | **CLOSED** — unbounded `Vec` → fixed-memory `hdrhistogram`; unit-guarded (`percentiles_from_histogram`) + live-proven (real merged percentiles). |
| `soak10x` scenario exists and is runnable | **CLOSED** — `soak10x.json` + `just perf soak10x` (caps raised for 3000 conns). The *run* is B2. |
| All six item-6 targets on the production-shaped env | **OPEN (B2)** — needs the pinned-hardware runs + the fix loop. |
| 24 h soak green | **OPEN (B2)** — the machinery it needs (bounded memory + the scenario) is now done; the run is operator work. |
| Every fix has a regression guard | **OPEN (B2)** — no fixes yet; the fix+re-measure loop is B2. |
| `reconnect-storm` / `hot-channel` / `call-churn` scenarios | **OPEN (B2)** — each needs a new protocol-driver mode, not just a config. |

### Reflection

- **The machinery seam held a third time.** Part A = the measurement machinery; B1 = the *last* piece of machinery (the store that survives the soak); B2 = the runs and the new drivers. B1 is the smallest honest unit — a flagged latent OOM plus the one scenario config that OOM would blow up on — and it is fully unit + live verifiable on the dev host with nothing pinned. The 24 h run stays out-of-band, like the fuzz burn-in.
- **Lazy is not "refuse every dependency."** Adding `hdrhistogram` reads as anti-ponytail until you cost the alternative: a correct reservoir sampler is *more* code and *less* accurate. Laziness is writing less *correct* code; `default-features = false` kept the cost to two tiny, already-vetted transitive crates, and the tool is the roadmap's own named choice.
- **Delegation's payoff isn't only catching defects.** Junior on the two disjoint files, senior on the histogram surgery, review before trust — and this time the review found the work clean. The win was the parallelism when the junior got it right, not a correction. Reviewing anyway is still the job.

### Not committed / next session

- **Sprint 10 part B1 is complete and green but untracked** on top of committed 0–9 + Sprint 10 part A. First commit lands the loadgen histogram swap (`Cargo.toml` / `Cargo.lock` / `driver.rs` / `main.rs` + the adapted percentile/classifier tests), `scenarios/soak10x.json`, the scenario-parameterized `just perf` + the raised pre-auth caps, and the `perf/results/README.md` update — the operator's call, as every sprint. Drift gate untouched.
- **Next: Sprint 10 part B2.** In order: (1) the three remaining scenarios, each needing a new *protocol driver* mode — `reconnect-storm` (drop → reconnect → resume with `last_seq`; `SeqTracker` already does the per-client continuity check *through* the reconnect), `hot-channel` (group-channel support: `channels.create` + one shared ~100-member channel for the fan-out shape), `call-churn` (a calls driver: `start`/`accept`/`signal`/`hangup` at ~1 Hz); (2) the pinned-hardware profiling passes (`pg_stat_statements` → `cargo flamegraph` → `tokio-console`), evidence-first per item 4; (3) the fix + re-measure loop (each fix its own commit with before/after numbers); (4) tighten the nightly smoke to measured-p99 + 50 % (item 7) — and move `perf-smoke.yml`'s runtime Core to `opn_app` at the same time (the part-A carry-over note); (5) the 24 h 10× soak run on the pinned stack.
- **Minor / flag for B2's first soak run:** the seed loop mints sessions *sequentially* (`for i in 0..conns { mint().await }`). At 300 conns (design/smoke) that is already the pattern; at 3000 it is 10× the sequential startup mints, which may want a batched/concurrent seed before the real soak. Only the 20-conn histogram path was exercised this session, so this is untested at soak scale. Non-blocking for B1.

### Follow-up (operator decision): a routine `soak-short`, 24 h stays release-only

The operator flagged the 24 h soak as too much. It is *not* cut: 24 h lives in OPN.md §15 + OPN-CORE.md §14 (design docs) and is the specific gate for this project's headline failure class (the "phone that degrades on day 3", the whole stability-over-velocity thesis), and it runs **once per release**, not routinely. Slow RSS creep (< 1 MB/h) and fd leaks are invisible in an hour and obvious in 24 h — that gap is exactly what the long run buys, so weakening it is weakening the release gate. Instead, per the operator's choice, the *routine* soak is now a real scenario: **`soak-short.json`** — **design** load (300 conns, 30 msg/s), 2 h. This implements Sprint 11 item 7's already-named "1 h mini-soak" (run a bit longer), at design load not soak10x's 10×/3000-conn load so it runs often on the RAM-limited dev host and reuses the design driver. At 2 h it is ~24× the 5-min nightly smoke — long enough to surface leaks/janitor accumulation the smoke can't, while `soak10x` (24 h, 10×) stays the once-per-release ritual. `just perf soak-short` runs it; added to the perf README scenario list. No design-doc amendment — this is additive (a new dev-facing scenario), and the release gate is untouched.

---

## 2026-07-20 (part A) — Sprint 10 **part A** (measurement foundation: the loadgen seq-continuity verifier + the `design` scenario + the core-pinned production-shaped perf stack + the `perf/results/` discipline): built, verified live (a healthy design-shaped run, `seq_gaps=0` end to end); the profiling/fix loop + the four heavier scenarios (part B) deferred

Sprint 9 closed the verification net. Sprint 10 is *measured proof* of the §7 targets — and it splits on the **same machinery-vs-measurement-run seam** every hardening sprint since 4 has taken: **(A)** the machinery that makes the measure→profile→fix loop *possible and self-checking* — the delivery guarantee turned into a continuously-checked property (the test-plan deliverable), the first real scenario (`design`), and the production-shaped, core-pinned environment + committed-results discipline the numbers must come from; **(B)** the actual profiling passes, the fix+re-measure loop, the tightened nightly gate, and the three heavier scenarios (`soak10x`, `reconnect-storm`, `hot-channel`, `call-churn`) — all of which need either genuinely new driver machinery or a long run on pinned hardware (operator work, the same shape as the 24 h soak and the fuzz burn-in). Part A is the self-contained, fully-locally-verifiable slice; the release-gate numbers are part B. No roadmap amendment — part A is item 1's `design` scenario + item 2 (measurement discipline) + item 3 (environment) + the test plan's "seq-continuity verifier … under all scenarios", verbatim.

### What exists now

- **loadgen seq-continuity verifier** (`crates/loadgen/src/driver.rs`) — a `SeqTracker` per subscribed channel, fed every received `channels.message` seq, plus a `ConnStats::seq_gaps` counter and a `Scenario::assert_no_seq_gaps` exit gate (mirrors the `error_acks`/`assert_error_acks` pattern exactly). It reports how many seqs are *missing* from the contiguous run `[first_seen .. max_seen]` — the delivery guarantee ("no acked message is lost") as a number, checked live under load. It is **order-insensitive by design** (see decision 1) and **O(reorder-window) memory, not O(messages)** (decision 2), so it is soak-safe. Surfaced in the JSON summary (`"seq_gaps"`) and the human table.
- **`crates/loadgen/scenarios/design.json`** (new) — Sprint 10 item 1's `design`: 300 conns, 30 msg/s, realistic mix (`typing_every=10`≈10 %, `read_every=20`≈5 %), `assert_no_seq_gaps: true` + `assert_no_durable_closes: true`. Groups deferred to the hot-channel slice — this is the paired-DM graph. The ack-p99 gate stays at the loose 25.0 (decision 3).
- **`smoke.json` gains `assert_no_seq_gaps: true`** — the nightly 5-min smoke now checks the delivery guarantee too, for free (the test plan wants continuity "under all scenarios").
- **`docker-compose.perf.yml`** (new) — production-shaped infra: Postgres 16 tuned `shared_buffers=1GB` (§7.1 wants 1–2 GB; the 6–8 GB dev host makes 1 GB the safe default), `max_connections=100`, `work_mem=16MB`; Redis + MinIO; no coturn (design/soak never call); separate `*_perf` volumes so perf data never touches dev data. Core is **not** in the compose — it runs as a host release binary pinned to the E-cores.
- **`just perf` + `PERF_CPUSET := "12-19"`** (justfile) — the one-command run: perf stack up → release build → Core started `TOKIO_WORKER_THREADS=4 taskset -c {{PERF_CPUSET}}` on the E-cores → healthz wait → mint tenant → run `design` → the JSON summary lands in `perf/results/<date>-design.json` with the exit code preserved → teardown. `PERF_CPUSET` is a documented hardware knob (decision 5).
- **`perf/results/`** (new, README + `.gitkeep`) — the measurement discipline (item 2): every run's JSON committed as `<YYYY-MM-DD>-<scenario>.json`, "the trend is the artifact, a single run is noise", generator-isolation on the E-cores, and the six item-6 gate targets listed.

### Decisions closed during implementation (roadmap deviations, all ponytail)

1. **Set-completeness, not in-order arrival — the load-bearing decision.** The naive reading of "seq gaplessness" is "assert seqs arrive `1,2,3,…` in order." That is **wrong against this system**: the send hot path fans out *post-commit and fire-and-forget* (`channels/mod.rs:89` — `publish` runs after `store::send_message` has already committed and released the channel row lock), so two concurrent senders whose txns commit as seq 1 then seq 2 can have their `publish` calls race and reach a subscriber as `2, 1`. OPN.md §5 makes this explicit — the client dedups/reorders by seq; the server guarantees only *no loss*. So `SeqTracker` asserts the received seq **set** is hole-free from first-seen to max-seen, tolerating transient reorder. Asserting in-order would false-positive on correct behavior and make the whole `design` gate un-shippable.
2. **O(reorder-window) memory, not a growing set.** `SeqTracker` keeps a `frontier` (highest fully-contiguous seq) + a small `pending` BTreeSet of seqs stuck above an open hole. The reorder window is microseconds-to-single-digit-ms (bounded by command latency), while the run's 2 s drain grace dwarfs it, so `pending` is empty at run end unless a seq was *genuinely* lost. This sidesteps the unbounded-`Vec` concern the module header flags for the 24 h soak — the continuity metric never accumulates per-message state.
3. **`design.json`'s ack-p99 gate stays at the loose 25.0, not item 6's 5 ms.** 5 ms is the *post-optimization* target; item 7 tightens the gate to measured-p99 + 50 % *after* the fix loop measures it. Asserting 5 ms now — before a single pinned-hardware run — would be asserting a threshold I have not measured (and the first colocated throwaway already showed ~50 ms on an unpinned box). The delivery invariants (`no_seq_gaps`, `no_durable_closes`) hold regardless of tuning and are asserted; the perf threshold waits for its measurement.
4. **`assert_no_seq_gaps` added to `smoke.json` too.** One line makes the nightly smoke a continuous delivery check, not just a latency check — the guarantee is watched every night, not only in the Sprint 10 measurement runs.
5. **E-core pinning is a `PERF_CPUSET` knob, not a constant.** The i5-14500 is hybrid (6 P-cores as logical 0–11 with HT, 8 E-cores as 12–19, no HT); pinning Core to the E-cores keeps loadgen + OS off the cores it measures (item 2's "colocated generator steals the CPU it is measuring"). But logical-CPU numbering is host-specific, so it is a documented calibration knob (`lscpu -e` to verify) — the hardware-needs-tuning instinct, applied to a cpuset.
6. **The perf Core runs as `opn_app` (RLS-enforced), not `opn_migrate`.** The delegated recipe first mirrored perf-smoke.yml, which runs its runtime Core as `opn_migrate` — the owner role that *bypasses* RLS. For a production-shaped measurement that quietly reports optimistic numbers: production runs as `opn_app` (CDR-3), so the per-query RLS-policy cost must be *inside* the measured path. Corrected to the same role split `chaos/lib.sh` uses (`opn_app` runtime, `opn_migrate` migrations only).
7. **Delegated the environment harness to a parallel opus subagent; kept the verifier + the design decisions.** The compose file, the `just perf` recipe, and the results README are mechanical infra/docs with clear references (dev compose, perf-smoke.yml, lib.sh) — a good junior task, run in parallel while I wrote the verifier. The out-of-order design call and the `SeqTracker` logic stayed with me. Reviewing the returned work caught decision 6.

### The keeper this session: a *design* keeper, not a code fix — the out-of-order-delivery realization, caught before the first assertion was written

Unlike the Sprint 9 slices (each a net over an existing guarantee, no keeper), this is a *build* slice and it had a real keeper — but a **design** one: the verifier's core assertion. Had I written the obvious "received seqs must be contiguous *in order of arrival*", the gate would trip the instant two paired senders raced — flagging a delivery **bug** that is in fact the documented design (post-commit fan-out + client-side reorder, OPN.md §5). The catch came from *tracing the real send path* (`channels/mod.rs` — `publish` is post-commit, fire-and-forget) **before** writing the assertion, not from a red run afterward. The permanent guard is the 5 unit tests, the load-bearing one being `seq_out_of_order_that_resolves_is_no_gap` (`1,3,2,4` → 0 gaps): its silent inversion — treating reorder as a hole — is exactly the false-positive that would make the design gate un-shippable. The negative control is `seq_genuine_hole_is_a_gap` + `seq_gaps_surface_through_handle_incoming` (a real dropped seq → nonzero `seq_gaps` → the exit gate trips): the gate is not vacuous, a lost message *does* fail it. A second, smaller keeper was the delegated-work review (decision 6): measuring with RLS bypassed would have reported a system that isn't production. Both are the kind of thing the §15 budget buys — one from reading the code before asserting, one from reading the subagent's output before trusting it.

### Verification (rule 4)

- `cargo fmt -p opn-loadgen --check` clean; `cargo clippy -p opn-loadgen --all-targets -- -D warnings` clean (the new `handle_incoming` arg needed one `#[allow(clippy::too_many_arguments)]`; `SeqTracker` is `match`/`BTreeSet`, no unwrap; the `core` crate's `unwrap_used` deny is untouched — no core change).
- **21 loadgen unit tests green** (16 prior + 5 new: `seq_in_order_no_gaps`, `seq_out_of_order_that_resolves_is_no_gap`, `seq_genuine_hole_is_a_gap`, `seq_duplicate_is_not_a_gap`, and `seq_gaps_surface_through_handle_incoming` — the last built from the real `ServerMsg::Push`/`Evt::ChannelsMessage` wire types, not hand-written JSON, so it rides the actual serialization).
- **Live end-to-end run against the real stack** (docker compose: Postgres 16 + Redis 7 + MinIO, a single release Core, on the dev host i5-14500): a healthy design-shaped run (20 conns, 20 msg/s, 15 s) → **320 sends / 640 recvs, `seq_gaps: 0`, 0 durable closes, 0 rate-limited, exit 0, PASS**. Non-vacuous: 640 messages flowed through `SeqTracker::observe` (each of 320 sends received by both pair members) and zero holes were found *despite* concurrent senders (reorder possible). The first throwaway (40 msg/s) exposed the `assert_ack_p99_ms` gate tripping at ~57 ms on the colocated, unpinned host — orthogonal to delivery, and itself the empirical case for the core-pinned perf stack this slice builds.
- `docker compose -f docker-compose.perf.yml config` parses clean; `just --list` shows the new `perf` recipe without a parse error.
- Drift gate untouched — `seq_gaps` is a loadgen-internal summary field, not a contracts wire type; no `.d.ts` changed.

### Exit criteria status (Sprint 10 — part A slice; part B = the profiling/fix loop + heavier scenarios still open)

| Criterion (roadmap Sprint 10) | Status |
|---|---|
| (test plan) seq-continuity verifier — per-topic gapless delivery under load | **CLOSED** — `SeqTracker` built, wired into `design` + `smoke`, proven live (`seq_gaps=0`) and unit-guarded (a real hole trips the gate). |
| All six targets met on the production-shaped environment, numbers committed to `perf/results/` | **OPEN (part B)** — the machinery is all in place (`design` scenario + core-pinned `just perf` stack + `perf/results/` discipline); the numbers need the pinned-hardware runs + the fix loop. |
| 24 h soak green | **OPEN (part B)** — needs the `soak10x` scenario (+ the hdrhistogram/reservoir sampling the module header flags) and a long operator run. |
| Every fix has a regression guard | **OPEN (part B)** — no fixes yet; the fix+re-measure loop is part B. The tightened nightly smoke (item 7) lands there. |

### Reflection

- **The machinery-vs-measurement seam was the right first cut.** Part A is buildable and *fully locally verifiable* — the verifier's unit tests and one short live run prove the machinery on the dev host. Part B needs the pinned hardware and the long runs (the actual p99/RSS/soak numbers), the same operator-out-of-band shape as the 24 h fuzz burn-in and the soak. Splitting there keeps the slice self-contained, exactly like every hardening sprint since 4.
- **A build slice's keeper is a design realization, not a code fix.** The value this session was reading the real fan-out path *before* writing the assertion and discovering that "seq gaplessness" must mean set-completeness, not arrival order. Getting it backwards would have shipped a release gate that fails on correct behavior — the most expensive kind of wrong, because it looks like rigor. The §15 budget buys the reading, not just the test.
- **Ponytail held, and delegation held with it.** The mechanical harness went to a parallel junior subagent; the subtle verifier and every design call stayed with me; reviewing the returned work caught the RLS-role inaccuracy that would have poisoned the numbers. Juniors on the harness, seniors on the invariant — and the review is part of the senior's job, not an afterthought.

### Not committed / next session

- **Sprint 10 part A is complete and green but untracked** on top of committed 0–9. First commit lands the loadgen seq-continuity extension (`driver.rs`/`main.rs` + 5 unit tests), `scenarios/design.json`, the `smoke.json` gate, `docker-compose.perf.yml`, the `just perf` recipe + `PERF_CPUSET`, and `perf/results/` (README + `.gitkeep`) — the operator's call, as every sprint. Drift gate untouched.
- **Next: Sprint 10 part B.** In order: (1) the remaining scenarios, each needing new machinery — `soak10x` (the hdrhistogram/reservoir sampling before it records hundreds of millions of samples, per the module header), `reconnect-storm` (a drop→reconnect→resume driver mode, verifying seq continuity per client through the reconnect — the `SeqTracker` already does the checking), `hot-channel` (group-channel support: `channels.create` + a shared 100-member channel), `call-churn` (a calls driver: `start`/`accept`/`signal`/`hangup`); (2) the profiling passes on the core-pinned stack (`pg_stat_statements` → `cargo flamegraph` → `tokio-console`), evidence-first per item 4; (3) the fix+re-measure loop (each fix its own commit with before/after numbers); (4) tighten the nightly smoke to measured-p99 + 50 % (item 7); (5) the 24 h 10× soak. Each is a clean slice on the same seam.
- Minor/non-blocking: `perf-smoke.yml` still runs its *runtime* Core as `opn_migrate` (RLS bypassed) — harmless for its loose 25 ms gate, but it should move to `opn_app` when item 7 tightens that gate, so the nightly numbers match production the way `just perf` now does. The `.gitkeep` in `perf/results/` is redundant now the README tracks the dir; harmless, left alone.

---

## 2026-07-20 — Sprint 9 **part B2b/3** (chaos, link-drop slice: a fake `/link` consumer as loadgen `--link-drop` + `http::get` + `chaos/link-drop.sh` on the shared `chaos/lib.sh` + `just chaos`/`chaos.yml` running **all four** drills): built, verified live (three consecutive green runs, one keeper caught on run one). **Sprint 9 is now CLOSED.**
